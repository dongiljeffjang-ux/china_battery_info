...
정책-기업 연결 웹 재검토, `pending_review`→`verified` 이관(SQL 적용·코드 정리 완료), Sankey 기본 기간, 보조 데이터 툴팁화,
순이익·R&D 시계열 표시와 툴팁 발행일(`b7f24db`, 푸시 대기). 공시 처리 순서는 바꾸지 않기로 했다(되돌림).
...
운영 커밋 7개(`67b7114` `11e6943` `c1bd84a` `297b959` `3f81ca6` `b368cc2` `02787f1`: 관리자 평가 화면 4건, 검색 수정 2회,
배치 평가 결함, 보조 데이터 토글), 기준선 표(현재 비교 기준 run `0cb59255`, 54문항, Hit@10 0.918 · MRR 0.676 · Recall@10 0.838),
문항 추가 방식(에이전트 초안 → 사용자 승인), 미커밋 도구 5개, 이 세션에서 틀렸다가 바로잡은 것 4건, 열린 일. **아래쪽(Codex)** 은
운영 반영 커밋 `c0afa8b` `2f09ac5` `acc7eb2` `5f08d06` `c32f9a1` `7efb36b`과 C0~C3 기록이다. Codex가 진행 중인 것은 시계열 정량
부속정보 숨기기. 검색 상수는 54문항 로컬 채점(`scripts/eval-benchmark-local.mjs`) 없이 바꾸지 않는다. 사람 검색 판정은 118건이다.

## 1. 운영 자산

### 최신 추가 작업 — 중국 정책축 (2026-09-10, 미배포)

Daily 정책 검색·본문 처리, 첨부 과거 정책 35건(발표·시행 일정 67항목), 기업분석·비교 시간축의 공통 정책 표시, 두 LLM 보고서의 정책 변수 입력을 구현했다. 비교 보고서에는 정책 분석과 당시 근거도 저장한다. 과거 웹 백필과 새 SQL은 필요 없다. 첨부의 원문확인 표기는 독립 검증으로 취급하지 않는다.

문법·모듈 로드·정책 검사·전체 회귀 47개 및 diff 검사 통과. 운영 배포·실제 수집·유료 보고서 생성·브라우저 레이아웃 검증은 미실시이며 현재 미커밋 상태다. 상세 파일·검증 범위·다음 작업은 `HANDOFF-CODEX.md` 맨 위의 정책축 절과 [POLICY-AXIS.md](POLICY-AXIS.md)를 참고한다. 아래 운영 자산은 기존 서비스를 가리키며 이번 정책축이 이미 배포됐다는 뜻은 아니다.
...
- 9월 pending 일괄 처리 완료: 300건 중 회사 미연결 14건을 제외한 276건 처리. `verified 220`, `not_pending 4`, `robots_disallowed 8`, `processing_failed 28`, `body_too_short 12`, `body_unavailable 4`.
- RAG 검색·질의 재작성 모델을 `gpt-5.6-luna`로 전환. 임베딩(`text-embedding-3-small`)과 뉴스 본문 구조화 모델은 유지.
- 기업 시간축 셀은 사업·기술 신호 우선 정렬, 저신호 항목 기본 접힘(`+N건 더보기`/`접기`)으로 배포. 커밋 `df8f8b3`, Production `china-battery-lens.vercel.app` READY.
- 다음 실행은 일반 수집 1회(오늘 기준 최근 3일 신규 뉴스)이며, 기존 URL·verified 상태는 보존된다.
