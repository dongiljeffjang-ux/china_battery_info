...
- CNINFO 공시 처리 순서 변경: 원인 보고만 했고 결정 대기.

### 확인했지만 손대지 않은 것

- **CNINFO 공시 분석 0건 → 원인 확인·수정.** `pipeline_log` 25개 훅 대조: 09-07 이후 모든 훅이 공시 4건을 선별했지만 줄 순서가 늘 11~14번째였고, 09-08 야간 훅 상한 2회 이후 한 훅이 5건 안팎만 처리해 한 번도 닿지 않았다(훅마다 선별을 새로 해 새 뉴스가 앞 10칸을 채운다). 09-07 상한 6회 때 처리된 8건이 지금까지의 공시 분석 전부다. **수정: 훅마다 공시 2건을 맨 앞에 둔다(`DISCLOSURE_FRONT_PER_HOP=2`, 하룻밤 4건).** 공시 유입은 하루 약 14건이라 백로그는 줄지 않고 점수 상위만 읽힌다(설계 의도: 회사별 소수만 핵심 이벤트). 뉴스 처리량은 그만큼 준다. 다음 야간 `pipeline_log`에서 공시 `ok`가 생기는지 확인. `HANDOFF.md` 9절 1항의 원인 설명은 낡았다.
