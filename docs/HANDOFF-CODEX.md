...
> **2026-09-08 저녁 — 벡터 지식 복구 계획 수립. 실행은 `docs/PLAN-embedding-recovery.md`를 따른다.**
>
> 오늘 오후 조사로 확인한 것: 정기보고서 이벤트 추출 재현율이 매우 낮고(완룬 MD&A 앞 12,000자에서
> 후보 28건 중 1건 저장), 임베딩 텍스트에 시점 정밀도가 빠져 결산일이 날짜로 읽히며(532/751건),
> 정량 테이블 6,410행은 검색 경로가 없다. 원인 진단(T0.1)부터 재처리(T3)까지 작업 그래프·위임
> 브리프·회귀 기준을 위 계획 문서에 적었다. `scripts/probe-digest.mjs`가 T0.1용 진단 스크립트다
> (DB 쓰기 없음, LLM 호출 있음, 아직 실행 안 함).

> **2026-09-08 최신 갱신 — 벡터DB(RAG) 사람 평가 화면 추가. 코드 완료, SQL 적용·배포 대기.**
>
> ## 무엇을 왜
>
