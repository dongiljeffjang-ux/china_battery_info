...
## 2026-09-14 Claude Code — Daily 선정 규칙: 단일 매체 단독 보도 배제를 완화 (사용자 지시)

`api/generate-daily.js`의 `DAILY_REPORT_PROMPT`가 "단일 제3자 언론 보도만으로 확정할 수 없는 주장은 고르지
않는다"고 못박아, 网通社 단독 보도(샤오미자동차-CALB·신왕다 협력, 09-12 발행, 본문 대조 통과)가 후보 8건에
있었는데도 Top 3에서 빠졌다. 사용자 확인: 후보는 이미 본문 대조를 거쳤으므로 단독 보도라는 이유만으로
배제하지 말고, 언론사 실명 보도이고 날짜·사실관계가 맞으면 중요도로 판단하라는 지시.

프롬프트를 "단일 매체 단독 보도라는 이유만으로 배제하지 않는다. 출처 불명 소문·루머·미검증 SNS·광고성
게시물일 때만 배제한다. 회사 직접 발표·공시가 신뢰도는 가장 높지만 유일한 기준은 아니다"로 바꿨다. 스키마·
구조 지시(`DAILY_REPORT_STRUCTURE_INSTRUCTION`)는 그대로다. 이 문구를 고정한 회귀 검사는 없어 직접 수정했다.
`node --check`·`npm run check`·`git diff --check`·`check-*.mjs` 62개 통과. 다음 Daily에서 단독 보도 채택
비율이 실제로 늘었는지 확인할 것.
...
- 기업 페이지의 `전략 진단`·`패턴 분석`·`전략 변수` 버튼을 **시계열 분석 보고서** 하나로 합쳤다.
- 생성 프롬프트는 출발점 → 변화의 축적 → 현재 위치와 반복 실행 방식 → 판단을 바꿀 핵심 변수 → `So What`을 한 중심 논지로 연결한다. 마지막에는 한국 셀·소재사의 경쟁 조건·고객 기회·공급망 선택 중 무엇이 달라지는지 답한다. 핵심 변수는 기존 `경로 A/B` 시나리오를 폐기하고 현재 사실·중요성·관측 신호만 제시한다.
- 일반적인 40~50개 시계열 팩트를 한 번에 활용한다. 48건을 넘을 때만 연도·레이어·기술 축을 보존해 압축하며, 기준 시점 대비·사업선별·진행 상태 보조표 세 개를 같은 입력에 제공한다.
- `reportMode` 클라이언트/API 분기를 제거했고 응답 호환 표시는 `report_mode: "timeline"`이다. `app.js` 캐시 버전은 `20260911-unified-timeline`.
- `npm run check`, 전체 `scripts/check-*.mjs`, `git diff --check` 통과. 로컬 접근 키 화면까지 브라우저에서 로드·콘솔 오류 없음·새 버튼 1개/구 버튼 0개를 확인했다. 실제 LLM 생성 품질은 배포 후 기업 한 곳으로 확인할 것.
...
### 한 줄 상태
커밋 9개 모두 `main` 푸시(마지막 `fe01856`). SQL `company-policy-link.sql` 적용 확인. **남은 사용자 작업 1건: 관리자 › 데이터 감사 › "신호 주체 재판정"을 한 번 더 눌러 규칙 v2 재판정(신호 32개·기사 25건)을 돌린다.** 새 프롬프트로 만든 실제 보고서 품질(요약·유보 표현·분량)은 아직 확인 전이다.

### 커밋 (시간순)

| 커밋 | 내용 |
|---|---|
| `bd9fd37` | **정책–회사 연결.** 정책 8건을 단어 일치로 골라 참고로 넣고 "관련 없으면 부록으로" 보내던 방식을 없앴다. `lib/policy-links.js`가 회사 사건 최근 120건과 정책을 보고 direct/indirect 연결을 추론, 입력 사건 번호를 근거로 요구하고 서버가 대조한다. 저장 `company_policy_link`(회사당 한 행). 보고서에는 연결된 정책만 사건과 같은 표로 넣는다. 비교 보고서의 정책 웹 재검토 호출은 삭제. **보고서 3모드 차별화**(모드별 골격·입력 보조표·강조색). **Daily 후보 창**(어제·오늘 발행분 ∪ 직전 Daily 이후 검증분 ∪ 직전 Top 10, 선정이 비면 기존 Daily 유지). **Daily 한 지면**(총평·사실·한국 소재사 관점을 01/02/03으로) |
| `17a8f0a` | **Sankey에 Top 10 기사 포함**(예전엔 뺐다 — 샨샨이 통째로 빠진 원인). 미검증 헤드라인은 원문 제목 일치 또는 같은 회사·테마·방향 ±2일·제목 단어 2개 이상 겹침이면 같은 소식으로 합친다. "가격 이미 인상"처럼 두 단어 사이 수식어 허용, 调涨 추가 |
| `2bb4c8d` | **보고서 프롬프트 전면 개정(사용자 지시).** 모든 보고서 맨 위 핵심 요약 3~4줄(`summary_ko`/`## 핵심 요약`). "판단할 수 없다"·인과 면책·출처 면책 문장 금지, 근거 없는 논점은 쓰지 않고 뺀다. 비교 보고서의 "판정 불가 사유 병기" 5곳 삭제, 불확실성은 `data_gaps_ko` 한 칸. 시계열 공통 지시문 15.3KB→4.4KB(사용자 허락으로 문장 재작성). 시계열 보고서는 서버가 유보 문장 수를 세어 3문장 넘으면 그 문장만 한 번 고쳐 쓴다(`lib/report-phrasing.js`). GB 표준번호 뒤 한국어 괄호 설명은 서버가 붙인다. 비교 화면 셀 표시 3→6건 |
| `5120390` | **비교 리포트 시간 초과 수정.** 확인: 초안 호출에 timeoutMs가 없어 기본 45초 → 14:02:55·14:03:43 연속 TimeoutError. 초안 80초, 요청 시작 기준 deadline 285초를 넘겨 검증은 남은 시간만 쓰고 40초 미만이면 초안(draft_only)으로 낸다 |
| `afab7a7` | Sankey **회사 선택**(밸류체인별 체크박스·신호 건수, 고른 회사는 12개사 자르기 없이 모두, 브라우저 저장). 정량 궤적 그래프 값 레이블 10.5→15px |
| `c6b7871` | 리포트 화면 글자 크기(요약 상자가 인쇄용 9·11px로 그려지던 문제 → 16px, 제목 32→26px). **보고서 정책을 생성 시점 ±3년으로 한정**(첨부 연혁 35→20건). 기업 화면 시간축은 전체 연혁 유지 |
| `38631e7` | Sankey 신호를 근거 문장에 이름이 나온 연결 회사에만 붙이는 임시 규칙(이후 `fba15e7`의 예비 규칙이 됨) |
| `fba15e7` | **신호 주체 판정(근본 수정).** 아래 "Sankey 신호 오귀속" 참고 |
| `fe01856` | 주체 규칙 v2: 완성차가 공급처·협력사로 정한 추적 회사는 확대 신호의 주체, 공급에서 빠진 회사는 축소 신호의 주체 |

### Sankey 신호 오귀속 — 원인과 수정 (운영 DB로 확인)
- **증상:** "닝더스다이 주가 하락의 배경"(byd·calb·catl 연결)의 CATL 축소 신호가 비야디·중촹신항에도 그려져 "기술·제품 축소 3건"(실제 1건).
- **원인:** `headline_signals`에 주체가 없어 기사에 연결된 모든 회사에 복사됐다. 신호 있는 검증 기사 241건 중 51건이 여러 회사 연결, 신호 124개가 선 380개로 복사. 이름 매칭으로도 51개(41%)는 산업 전반 수치·추적 외 회사(샤오미·체리·Xpeng·완화화학)·한국어 표기 차이(후난위능·금천루이샹·푸린징궁·흔왕달)·주어 생략 때문에 못 가렸다.
- **수정:** `lib/signal-subjects.js`. 기사 분석·검증 스키마에 신호별 `subject_scope`(company/industry/other_company)·`subjects_ko`, 모델 입력에 `[연결 회사]` 목록. 서버가 이름을 연결 회사 id로 되돌려 `subject_company_ids`·`subject_rule_version` 저장. Sankey는 모델 주체 1순위, 산업 전반·추적 외 회사는 회사 선 없음, 판정 없는 기사만 이름 매칭.
- **기존 기사 재판정 결과(15:11~15:14, 호출 29회):** 신호 513개 전부 판정. company 425 · industry 56 · other_company 32 · unresolved 0. 문제 기사들 기대대로 바뀐 것 확인.
- **남은 것:** 규칙 v2 재판정은 버튼을 다시 눌러야 한다(대상은 v1에서 other_company로 판정된 32개만).

### 운영 DB에 한 쓰기
- `company_policy_link`: 보고서 생성 시 정책 연결 판정 저장(자동).
- `article.headline_signals`: 재판정이 주체 필드만 더했다(신호 문장·방향·키워드 불변). 실행 전 대상 225건·신호 513개를 SELECT로 확인.
- 그 밖의 DELETE/UPDATE 없음.

### 확인했지만 손대지 않은 것
- **테마 오분류:** CATL "고급 배터리 주문" 축소(고객 이탈)가 근거 문장의 "6월 **출시**된"의 '출시' 때문에 기술·제품 테마로 접힌다. `themeOfKeyword`가 키워드보다 근거 문장 단어에 끌려간다. 사용자 결정 대기.
- 정책 창 ±3년 경계로 쌍적분 2차 개정(2023-06-29)이 빠진다. 3년 반으로 늘릴지 사용자에게 물었으나 답 없음.
- 비교 보고서는 유보 문장 서버 고쳐 쓰기가 없다(프롬프트 규칙만).
- 로컬 `.env`의 Supabase 키는 가림 처리돼 로컬 스크립트로 운영 DB를 읽을 수 없다. 점검은 Supabase MCP SQL로 했다.

### 다음 작업자가 할 일
1. 사용자가 "신호 주체 재판정"을 다시 누른 뒤: `subject_rule_version=2` 비율, other_company 32개 중 company로 바뀐 수, 중촹신항 룽자 배터리 확대 선 생성 확인.
2. 새 프롬프트 첫 보고서들 확인: 핵심 요약 3~4줄, 유보 문장 수(`[TIMELINE_REPORT]` 로그의 `hedge`), 분량, 정책 연결 표·GB 괄호.
3. 비교 리포트 `compare_report_draft` TimeoutError 재발 여부(`pipeline_log`).
4. 새로 분석되는 기사의 `headline_signals`에 주체 필드가 붙는지(분석·검증 둘 다 스키마 필수라 없으면 호출 실패로 보인다).

### 이번 세션에서 배운 것
- **한 건을 고쳤으면 전체에서 같은 유형을 세고 나서 "고쳤다"고 말한다.** 샤오미 사례 하나로 이름 매칭을 넣고 끝냈다가, 전수 점검에서 41%가 그대로인 것을 사용자 지적 후에야 확인했다.
- **LLM 호출에는 timeoutMs를 명시한다.** 기본값(45초)이 모델·입력 변화로 조용히 부족해졌다.
- **인쇄용 CSS가 화면 패널에 스코프로 들어간다.** 인쇄용 크기(9~11px)를 새 요소에 쓰면 화면에서도 그 크기로 보인다. 화면용 덮어쓰기를 같이 둔다.
...
| `0018fb5` | Daily Sankey 기본 기간: 평일 당일~**2일 전**(전에는 1일 전). 금요일 3일 전은 유지(사용자 지정) |
| `97632bd` | **보조 데이터는 칸에 제목만.** 기업 시계열·비교 칸에서 보조 항목의 요약 줄·수치 줄을 빼고 툴팁으로 보낸다(발생 법인·원문 링크·다른 근거 표시는 유지). 리포트 입력에는 툴팁과 같은 사실 전문·발생 법인·출처를 넘긴다(발생 법인 추가, 비교 리포트 사실 절단 200→300자 — 보조 사실 최대 227자 실측) |
| `6fa4e28` | `pending_review` 2단계: SQL 적용 확인 뒤 옛 값 읽기를 모두 뺐다. `scripts/check-verification-status.mjs`가 `api`·`lib`·`app`에 옛 이름이 다시 들어오면 실패한다 |
| `3d1aace` → `e945dfd` | 공시를 뉴스보다 먼저 처리하는 수정을 배포했다가 **사용자 요청으로 되돌렸다**(아래 "사용자 결정") |
| `b7f24db` | **(로컬, 미푸시)** 시장 레이어 정기보고서의 **순이익·R&D를 시계열 핵심으로 표시**(`lib/timeline-visibility.js` `REPORT_TIMELINE_QUANT`). 운영 이벤트 2,051건 대조: 18개사 73건 새로 보임(순이익 50·R&D 20·특허 곁들인 사업 이벤트 3), 새로 숨는 것 0. 특허는 기술 레이어라 원래 보였다. 모든 이벤트 툴팁 출처 줄에 **기사 발행일과 출처 주소**를 붙인다(`article.published_at` 조회 추가, 툴팁 링크는 누를 수 없어 글자로 둔다). 캐시 `app.js?v=20260910-source-date` |
| `fd08451` | **(로컬, 미푸시)** 사용자 요청: 시계열 리포트 본문의 맨 주소·주소 라벨 링크를 `link`로 줄여 새 창에서 연다(`inlineMarkdown`). 툴팁에서는 출처 주소를 다시 뺐다(발행일만). 리포트 입력의 URL은 링크를 만들 재료라 유지. `app.js`의 NUL 구분자를 `\u0000` 이스케이프로 바꿨다(grep이 바이너리로 보던 문제). 캐시 `app.js?v=20260910-report-link`. 이어서 리포트 입력에 기사 발행일을 넣었다(시계열 셀 `(사건 … · 발행 …)`, 비교 줄 끝 `· 발행 …`; 발행일 없는 보고서 이벤트는 그대로). 캐시 `app.js?v=20260910-report-date` |

### 운영 상태와 순서

1. **배포:** `e945dfd`까지 운영 반영 확인(20:08 Ready). **`b7f24db`·`fd08451`은 로컬 커밋만 — 푸시 전.**
2. **SQL 두 개 적용 완료(사용자 실행, DB 확인).** `evidence_alternative` 존재·RLS·anon 차단·service_role 쓰기. 기사 상태 `verified` 246 / `pending_review` 0, `admin_overview()`는 `verified`만 쓴다.
3. 배포 뒤 첫 후난위넝 비교 리포트(정책 포함)로 확인할 것: 기술 축 채워짐, 교정 라벨 "다른 근거로 DB에 병기", "정책–기업 연결 재검토" 목록, 보조 데이터 칸이 제목만 보이는지. 입장 키·비용 때문에 에이전트는 돌리지 않았다.
4. 오늘 밤 수집 로그에서 정책축(Codex 절)의 정책 검색·`china-policy` 이벤트 저장 확인.

### 결정 대기

- **리포트 입력에 기사 발행일 추가 — 구현함(`sourceDate`).** 시계열 리포트 셀을 `(사건 2026-08-18 · 발행 2026-08-20 · 출처: …)`로, 비교 리포트 줄 끝을 `출처: … · 발행 …`으로. 회고 기사(09-04 기사가 03-05 발표를 다룬 BYD 블레이드 사례)를 모델이 구분하게 하려는 것. 이벤트당 약 15자.
- 발표 자료 `docs/PRESENTATION-2026-09-16.md`·`.pptx` 커밋 여부.

### 남은 개선 과제 (이전부터)

검색 3차(× 4문항: 나트륨 특허·전고체 특허·전구체 합작·모로코 — 모로코는 보류), 질문 로깅(실제 Q&A 질문 미저장), RAGAS 점수화(답변 저장 → Python 채점 → run별 표), 기술 부채(레이어 미분류 112·시점 근거 없음 140, `report_chunk` 0건, 사업 관계망 화면, 여러 줄 표 오독).

### 사용자 결정 (2026-09-10)

- 시계열 정량 부속정보 숨기기: **재무 실적 중 중요 부분과 R&D·특허 수치는 핵심으로 남긴다.** Codex가 `minor` 값 대신 화면 필터(`lib/timeline-visibility.js`, `c32f9a1`)로 구현해 두었고, 순이익·R&D가 숨겨지던 것을 `b7f24db`에서 고쳤다.
- 툴팁 출처 줄에 발행일·출처 주소를 붙인다(승인).
- 원문 보관: 언론 기사는 번역본만, 연차보고서 등 공개 보고서는 원문 보관 — 이미 그렇게 구현돼 있음을 확인했다.
- HKEX 수집 경로(CALB·REPT): 기각.
- 검색 어휘(`셀사` 세그먼트)·모로코 문항: 보류.
- 웹 검증 교정은 원래 값을 덮지 않고 "다른 근거"로 함께 보인다. 다른 근거는 이후 리포트 입력에도 넣는다.
- 정책-기업 연결 재검토의 LLM 호출 1회 추가 비용 승인.
- 보조 데이터는 화면에서 제목만, 리포트에는 툴팁 내용 전체.
- **CNINFO 공시 처리 순서는 바꾸지 않는다.** 공시 내용은 이미 다른 경로(정기보고서 digest 등)로 반영돼 있다는 사용자 판단. 공시를 뉴스 앞에 두는 수정(`3d1aace`)을 배포했다가 사용자 요청으로 되돌렸다. 뉴스 처리량을 공시에 나눠 주지 않는다.

### 확인했지만 손대지 않은 것

- **CNINFO 공시의 기사 경로 분석은 09-07 이후 0건 — 의도적으로 둔다.** `pipeline_log` 25개 훅 대조: 모든 훅이 공시 4건을 선별하지만 순서가 11~14번째라 야간 훅 상한 2회에서 차례가 오지 않는다(확인). 사용자 결정으로 고치지 않는다(위 "사용자 결정"). 미시도 공시가 `pending`으로 쌓이는 것은 정상 상태로 본다. `HANDOFF.md` 9절 1항의 원인 설명은 낡았다.
