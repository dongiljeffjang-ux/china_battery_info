...
| `fd08451` | **(로컬, 미푸시)** 사용자 요청: 시계열 리포트 본문의 맨 주소·주소 라벨 링크를 `link`로 줄여 새 창에서 연다(`inlineMarkdown`). 툴팁에서는 출처 주소를 다시 뺐다(발행일만). 리포트 입력의 URL은 링크를 만들 재료라 유지. `app.js`의 NUL 구분자를 `\u0000` 이스케이프로 바꿨다(grep이 바이너리로 보던 문제). 캐시 `app.js?v=20260910-report-link` |
...
- **리포트 입력에 기사 발행일 추가(제안만, 미구현).** 시계열 리포트 셀을 `(사건 2026-08-18 · 발행 2026-08-20 · 출처: …)`로, 비교 리포트 줄 끝을 `출처: … · 발행 …`으로. 회고 기사(09-04 기사가 03-05 발표를 다룬 BYD 블레이드 사례)를 모델이 구분하게 하려는 것. 이벤트당 약 15자.
