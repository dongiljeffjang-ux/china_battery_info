...
일일 정책 검색(최근 3일, OpenAI·DeepSeek 각 1회) → URL·날짜 검사 → 내부 정책 분류로 기사 연결 → 정책 별도 본문 후보(최대 4건/훅) → 원문 추출·Daily·이벤트 저장. 기업 API에서 신규 정책 이벤트와 첨부 과거 연혁을 합쳐 회사 시간축·비교 시간축·두 보고서에 제공한다. 정책 검색 요청 수도 수집 예산에 포함한다. 자세한 경계와 출처는 `docs/POLICY-AXIS.md` 참고.
...
flowchart LR
  ...
  ARTICLE -->|한국시간 3일 전 00시부터 뉴스 상위 10건 + 과거 미시도 4건<br>공시 별도 몫| PROCESS[/LLM 본문 처리<br>process-article/]
...
수집은 공개 트리거를 허용하고, 비용이 발생하는 본문 LLM 처리·Daily 생성은 Vercel Cron의 `CRON_SECRET` 인증 요청에서만 실행한다. 뉴스 원문은 사용자의 명시 요구로 `article.body_original`에 보관하며 청킹·임베딩에 사용한다. 최근 뉴스 선별 창은 실행 시각 기준 72시간이 아니라 한국시간 3일 전 00:00부터 계산한다. 날짜만 확인된 기사가 DB에서 그 날 `00:00Z`로 저장돼도, 같은 한국 날짜 기사 전체가 처리 대상에 남는다. 창 밖의 기존 `pending` 뉴스도 매 회차 4건씩 별도 복구해, 원문 검증 성공 시 일반 기사와 동일하게 청킹·임베딩·Sankey 입력으로 활용한다. 원문을 확보하고 원문 기반 구조화 추출을 마친 기사는 `verified`로 저장한다. 원문을 읽지 못하거나 본문이 짧고, 검색 결과 날짜와 원문 날짜가 크게 다른 기사는 표시 대상에서 제외한다. 수집이 DB 오류로 중단되면 크론은 Daily를 다시 만들지 않는 본문 복구 홉을 이어, 이미 저장된 `pending` 기사가 다음 날까지 방치되지 않게 한다. 일시적 fetch 오류만 1시간·6시간 간격으로 최대 세 번 재시도하며 robots 차단과 본문 품질 기준은 완화하지 않는다.
