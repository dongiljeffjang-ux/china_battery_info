...
async function fetchSinaFinance() {
  const pages = await Promise.all([1, 2].map(async (page) => {
    ...
    return (payload.result?.data || []).map((item) => ({
      source: "Sina Finance", title: clean(item.title), url: item.url || item.wapurl,
      publishedAt: unixDate(item.ctime || item.intime), snippet: clean(item.intro || item.summary || item.wapsummary), kind: "news"
    })).filter((item) => item.title && item.url);
  }));
  return pages.flat();
}
...
async function fetchCatlNewsroom() {
  ...
  return items.map((match) => ({
    source: "CATL Newsroom", title: clean(match[2]), url: new URL(match[1], "https://www.catl.com").toString(),
    publishedAt: clean(match[3]), snippet: "宁德时代 공식 뉴스룸 발표", kind: "news", companyId: "catl"
  })).filter((item) => item.title && item.url);
}

function cninfoDateRange(days = 14) {
  const end = new Date();
...
async function fetchCninfo(company) {
  const params = new URLSearchParams({
    pageNum: "1", pageSize: "20", tabName: "fulltext", column: company.cninfo.column,
    stock: "", searchkey: company.cninfo.query, secid: "", plate: "", category: "", trade: "",
    seDate: cninfoDateRange(), sortName: "", sortType: "", isHLtitle: "true"
  });
  const response = await fetch("https://www.cninfo.com.cn/new/hisAnnouncement/query", {
    method: "POST",
    headers: {
      "User-Agent": "Mozilla/5.0 (compatible; ChinaBatteryLens/0.1)", "Referer": "https://www.cninfo.com.cn/",
  ...
  return (payload.announcements || []).map((item) => ({
    source: "CNINFO Disclosure", title: clean(item.announcementTitle),
    url: item.adjunctUrl ? `https://static.cninfo.com.cn/${item.adjunctUrl}` : "",
    publishedAt: item.announcementTime ? new Date(item.announcementTime).toISOString() : new Date().toISOString(),
    snippet: `${company.name_zh} (${item.secCode || ""}) 공식 공시`, kind: "disclosure", companyId: company.id
  })).filter((item) => item.title && item.url);
}

export async function discoverChinaSources() {
  const [sina, chinaNews, peopleNews, catlNews, ...disclosures] = await Promise.all([
    fetchSinaFinance(), fetchRss(CHINA_NEWS_RSS, "China News Finance"), fetchRss(PEOPLE_FINANCE_RSS, "People's Daily Finance"),
    fetchCatlNewsroom(),
    ...COMPANIES.map(fetchCninfo)
  ]);
