...
export default async function handler(request, response) {
  ...
  try {
    ...
    const [reports, top10, companyNews, pendingNews, flowEvents] = await Promise.all([
      ...
      dashboardQuery("sankey", `article?select=id,title_ko,title_original,published_at,keywords_ko,headline_signals,is_top10,verification_status,article_company(company_id)&published_at=gte.${from}&published_at=lte.${to}&verification_status=in.(pending_review,approved)&is_top10=eq.false&order=published_at.desc&limit=500`),
    ]);
