...
- 예외: Daily 리포트의 `insight_ko`와 기업 분석·비교 보고서의 해석 영역은 근거 기반 추론을 허용한다.
  복수 시점·레이어의 확인된 사실을 연결해 전략 변화와 한국 배터리사·소재사에 대한 사업적 함의를 쓸 수
  있다. 기업 분석 보고서는 진행 중인 사건에서 직접 이어지는 조건부 분기 해석도 쓸 수 있다. 근거에 없는
  사실·거래·고객을 만들거나, 시간적 선후를 인과로 단정하거나, 계획·협력·수주를 출하·매출로 격상하거나,
  투자 추천을 쓰는 일은 여전히 금지다. 사실과 해석은 화면 영역에서 분리한다.
- 원문 URL·매체·날짜·원문 발췌를 유지한다.
- 자동 본문 대조 통과 상태는 `verified`다(2026-09-10 이전 이름 `pending_review`, 운영 DB 이관 완료). 사람 승인을 뜻하지 않는다.
