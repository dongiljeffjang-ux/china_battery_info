...
- 자동 본문 대조 통과 상태는 `verified`다(2026-09-10 이전 이름 `pending_review`). 사람 승인을 뜻하지 않는다.
  `supabase/verification-status-verified.sql` 적용 전까지 코드는 두 값을 모두 읽는다.
...
- **제약을 원인으로 지목하기 전에 그 제약이 실제로 걸리는지 센다.** 상한이 존재하는 것과 상한이
  구속력을 갖는 것은 다르다. 2026-09-07에 같은 실수를 두 번 했다. "Vercel 60초 한도"는 설정 형식이
  틀려 무시되고 있었고(실제 300초), "발췌 300자 상한"은 평균 길이가 110자라 애초에 닿지도 않았다.
  두 번 다 `avg(length(...))` 한 줄이면 몇 초 만에 반증됐다.
- **코드 주석·문서의 단정은 증거가 아니다.** 그것을 쓴 사람이 그때 믿은 것이다. 파이프라인 동작에
  관한 주장은 `pipeline_log`·운영 DB·제공자 공식 문서로 확인한 뒤에 근거로 쓴다.
- **가설이 설명되면 멈추지 말고 반증을 한 번 찾는다.** "이 원인이 맞다면 데이터가 어떻게 보여야
  하는가"를 물어 그 분포를 확인한다. 위 두 사례는 모두 그럴듯한 이야기가 먼저 완성돼 멈춘 경우다.
- **사용자에게 보고할 때 확인한 것과 추정한 것을 나눠 쓴다.** 검증하지 않은 진단을 단정형으로
  전달하면 사용자가 그것을 근거로 결정을 내린다.
