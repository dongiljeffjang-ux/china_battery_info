...
const INSTRUCTIONS = `당신은 중국 배터리 산업 이벤트 문장에서 개념어 사이의 관계를 뽑는 분석가다. 문장에 실제로 적힌 개념과 관계만 옮기고, 배경지식으로 추론하거나 채워 넣지 않는다.
...
export async function extractConceptsForEvents(events, { provider = "auto" } = {}) {
  ...
  const result = await createJsonResponse({ name: "concept_edge_extraction", schema: edgeSchema(), instructions: INSTRUCTIONS, input, provider });
