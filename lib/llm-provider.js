...
// DeepSeek 웹 검색은 Anthropic 호환 엔드포인트로 보낸다. Responses API는 2026-09-10 V4.1 Flash 전환 뒤
// 내장 web_search를 무시하지만(문서 호환표 Ignored), 이 경로는 2026-09-14 실호출에서 server_tool_use 2회·
// 검색 결과 20건·당일(09-13·14) 발행 기사 3건을 실제로 돌려줬다. scripts/probe-deepseek-anthropic.mjs.
export const DEEPSEEK_ANTHROPIC_URL = "https://api.deepseek.com/anthropic/v1/messages";
export const DEEPSEEK_SEARCH_TOOL = Object.freeze({ type: "web_search_20250305", name: "web_search", max_uses: 8 });

export function llmConfig(provider = "auto") {
  // 근거 검색은 일반 수집 모델과 같은 OpenAI 키를 쓰되, RAG 답변 품질을 위해 Luna를 기본 사용한다.
  if (provider === "openai_rag" && process.env.OPENAI_API_KEY) {
    return { provider: "openai", route: "rag", url: OPENAI_URL, apiKey: process.env.OPENAI_API_KEY, model: process.env.OPENAI_RAG_MODEL || "gpt-5.6-luna" };
  }
  ...
  if ((provider === "auto" || provider === "deepseek") && process.env.DEEPSEEK_API_KEY) {
    return {
      ...
      // 검색은 Anthropic 호환 경로의 정식 ID를 쓴다. 그 경로는 모르는 이름을 deepseek-flash로 매핑하므로
      // 옛 별칭을 넣어도 같은 모델이지만, 응답 model 필드와 로그가 일치하도록 정식 ID를 기본으로 둔다.
      searchModel: process.env.DEEPSEEK_SEARCH_MODEL || "deepseek-flash",
...
// Anthropic Messages 응답을 Responses 모양으로 바꾼다. server_tool_use → web_search_call(검색 횟수),
// web_search_tool_result의 URL → action.sources(인용 대조), text 블록 → 마지막 message(최종 JSON).
// thinking 블록은 버린다. stop_reason=max_tokens는 incomplete_details로 남겨 잘린 응답을 구분한다.
export function anthropicToResponsesPayload(message) {
  const blocks = Array.isArray(message?.content) ? message.content : [];
  const output = [];
  const texts = [];
  for (const block of blocks) {
    if (block.type === "server_tool_use") output.push({ type: "web_search_call", status: "completed", action: { query: block.input?.query || null, sources: [] } });
    else if (block.type === "web_search_tool_result") {
      const sources = (Array.isArray(block.content) ? block.content : []).map((item) => item?.url).filter(Boolean).map((url) => ({ url }));
      const call = output.findLast((item) => item.type === "web_search_call");
      if (call) call.action.sources.push(...sources); else output.push({ type: "web_search_call", status: "completed", action: { sources } });
    } else if (block.type === "text" && typeof block.text === "string" && block.text.trim()) texts.push(block.text);
  }
  if (texts.length) output.push({ type: "message", content: [{ type: "output_text", text: texts.join("\n") }] });
  return {
    id: message?.id, model: message?.model, status: message?.stop_reason === "max_tokens" ? "incomplete" : "completed",
    incomplete_details: message?.stop_reason === "max_tokens" ? { reason: "max_output_tokens" } : null,
    output, usage: message?.usage || null, error: message?.error || null,
  };
}
...
async function callJsonResponse({ instructions, input, schema, name, webSearch = false, provider = "auto", timeoutMs, skipTelemetry = false }) {
  ...
  if (config.provider === "deepseek" && !webSearch) body.reasoning = { effort: config.thinking ? "low" : "none" };
  const deepseekSearch = config.provider === "deepseek" && webSearch;
  if (webSearch && !deepseekSearch) {
    body.tools = [{ type: "web_search" }];
    // 검색 전용 호출에서 auto를 쓰면 모델이 도구를 부르지 않고 학습된 기억만으로 JSON을 반환할 수 있다.
    body.tool_choice = { type: "web_search" };
    body.include = ["web_search_call.action.sources"];
  }
  // DeepSeek 검색은 Anthropic Messages 형식이다. 응답은 아래에서 Responses 모양으로 바꿔 뒤 처리를 공유한다.
  const requestUrl = deepseekSearch ? DEEPSEEK_ANTHROPIC_URL : config.url;
  const requestHeaders = deepseekSearch
  ...
  const requestBody = deepseekSearch
    ? { model, max_tokens: 6000, system: body.instructions, messages: [{ role: "user", content: input }], tools: [{ ...DEEPSEEK_SEARCH_TOOL }] }
    : body;
  let upstream;
  try { upstream = await fetch(requestUrl, {
    method: "POST",
    headers: requestHeaders,
    body: JSON.stringify(requestBody),
    // DeepSeek 검색은 추론 블록과 검색 2~4회를 한 응답에 담아 오래 걸린다. 수집 함수 한도(300초) 안에서 넉넉히 준다.
    signal: AbortSignal.timeout(timeoutMs || (deepseekSearch ? 55000 : webSearch ? 35000 : 45000))
  }); } catch (error) {
  ...
  const payload = deepseekSearch ? anthropicToResponsesPayload(await upstream.json()) : await upstream.json();
