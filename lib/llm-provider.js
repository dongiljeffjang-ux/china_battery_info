...
async function callJsonResponse({ instructions, input, schema, name, webSearch = false, provider = "auto", timeoutMs }) {
  ...
  try {
    ...
    if (config.provider !== 'deepseek' || !webSearch || !llmConfig('openai') || remaining < 3000 || outputText.length > 24000) {
      await logPipeline('llm', { ...telemetry, error: 'INVALID_JSON_OUTPUT', recovery: 'not_attempted' }, { status: 'failed' });
    ...
    try {
      ...
      await logPipeline('llm', { ...telemetry, error: 'JSON_RECOVERY_FAILED', recovery_error: error.message }, { status: 'failed' });
