...
// 신규 수집·백필에 집중할 핵심 25개사. COMPANIES는 과거 데이터 조회와 회사 귀속을 위해
...
export const TRACKED_COMPANY_IDS = new Set([
  ...
  "wanrun-new-energy", "lopal", "reshine", "youshan",
...
export function buildSearchGroups(provider, pilot = false, bootstrapCompanyIds = []) {
  ...
  const normalGroups = chains.flatMap(({ label, tag }) => {
    let ids = TRACKED_COMPANIES.filter((company) => company.type_tags.includes(tag) && !bootstrapSet.has(company.id)).map((company) => company.id);
  ...
  const bootstrapGroups = TRACKED_COMPANIES
...
export function plannedSearchRequests(pilot = false, bootstrapCompanyIds = []) {
  return ["openai", "deepseek"]
    .filter((provider) => llmConfig(provider))
    .reduce((total, provider) => total + buildSearchGroups(provider, pilot, bootstrapCompanyIds).length, 0);
}
...
async function discoverWebSearchNews(provider, pilot = false, bootstrapCompanyIds = []) {
  ...
  const groups = buildSearchGroups(provider, pilot, bootstrapCompanyIds);
...
export async function discoverChinaSources({pilot = false, bootstrapCompanyIds = []} = {}) {
  webSearchOutcomes = [];
  const selected = TRACKED_COMPANIES.filter(company => company.cninfo && (!pilot || ['catl','hunan-yuneng','btr'].includes(company.id)));
  const labels = ["web_search_openai", "web_search_deepseek", "catl_newsroom", ...selected.map((company) => `cninfo:${company.id}`)];
  const collectors = [discoverWebSearchNews("openai",pilot,bootstrapCompanyIds), discoverWebSearchNews("deepseek",pilot,bootstrapCompanyIds), fetchCatlNewsroom(), ...selected.map(fetchCninfo)];
...
export function companiesFor(item) {
  if (item.companyId) return TRACKED_COMPANIES.filter((company) => company.id === item.companyId);
  const corpus = `${item.title} ${item.snippet}`.toLowerCase();
  return TRACKED_COMPANIES.filter((company) => companyAliases(company).some((alias) => corpus.includes(alias.toLowerCase())));
}
