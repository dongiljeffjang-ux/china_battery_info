  ...
  const results = await Promise.all(groups.map(async (group) => {
    try {
      ...
      const result = await createJsonResponse({
        ...
        instructions: "중국 이차전지 산업 뉴스 리서처다. 웹 검색을 이용한다. 검색 결과에 실제로 제시된 원문 기사 URL만 반환한다. URL·제목·매체·날짜를 추정하거나 만들어내지 않는다. 광고, 자동차 소비자 리뷰, 주가 단신은 제외한다. 중국어/영어 원문 모두 가능하다. 대괄호 안은 서비스의 한국어 표준명과 중국어·영어 검색 별칭이다. 그룹 회사는 모회사뿐 아니라 명시된 주요 계열사 관련 기사도 찾되, 관계가 불명확한 동명사는 포함하지 않는다.",
