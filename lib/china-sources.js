...
// 검색 요청 하나에 넣는 회사 수 상한. discoverChinaSources 호출자가 예산 계산에 쓸 수 있게 내보낸다.
export const SEARCH_GROUP_SIZE = 4;
...
async function discoverWebSearchNews(provider, pilot = false) {
  ...
  if (pilot) groups = groups.map(group => ({...group, ids:group.ids.filter(id=>['catl','hunan-yuneng','btr'].includes(id))}));
  // 밸류체인 하나에 회사 10~12곳을 한 요청에 넣으면 모델이 검색만 반복하다 35초 안에 JSON을 못 낸다
  // (2026-09-06 전체 모드 로그: DeepSeek 3그룹 전부 실패, OpenAI 1그룹 타임아웃). 회사 수를 요청당
  // SEARCH_GROUP_SIZE 이하로 쪼개 요청 하나가 짧게 끝나게 한다. 요청 수는 ingestion-guard 예산이 막는다.
  groups = groups.flatMap((group) => {
    const chunks = [];
    for (let index = 0; index < group.ids.length; index += SEARCH_GROUP_SIZE) chunks.push(group.ids.slice(index, index + SEARCH_GROUP_SIZE));
    return chunks.map((ids, index) => ({ label: chunks.length > 1 ? `${group.label} ${index + 1}/${chunks.length}` : group.label, ids }));
