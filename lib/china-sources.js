...
// 검색 "레인"과 "엔진"을 나눈다. 레인은 무엇을 찾는가(글로벌 주요 매체 / 중국 현지 출처)이고, 엔진은
// 어느 LLM의 웹 검색 도구로 찾는가다. 2026-09-10 DeepSeek V4.1 Flash 전환 뒤 DeepSeek Responses API가
// 내장 web_search 도구를 무시한다(공식 문서 호환표 "Ignored", 운영 로그 2026-09-09 23:01 KST까지
// search_calls 5~17회 → 09-10 14:16 KST부터 전부 0회). 그래서 중국 현지 레인의 기본 엔진을 OpenAI로
// 옮겼다. 공급자가 검색을 되살리면 CHINA_LOCAL_SEARCH_ENGINE=deepseek 로 코드 배포 없이 되돌린다.
export const SEARCH_LANES = ["openai", "china_local"];
export function searchLaneEngine(lane) {
  if (lane === "china_local") {
    const engine = String(process.env.CHINA_LOCAL_SEARCH_ENGINE || "openai").trim().toLowerCase();
    return engine === "deepseek" ? "deepseek" : "openai";
  }
  // 엔진 이름을 직접 준 경우(진단 스크립트·과거 호출)는 그대로 돌려준다.
  return lane === "deepseek" ? "deepseek" : "openai";
}
// 정책 검색 프롬프트는 레인과 무관하므로 엔진마다 한 번만 부른다. 두 레인이 같은 엔진이면 1회다.
export function policySearchEngines() {
  return [...new Set(SEARCH_LANES.map(searchLaneEngine))].filter((engine) => llmConfig(engine));
}
const PILOT_COMPANY_IDS = ['catl', 'hunan-yuneng', 'btr'];
export const POLICY_COMPANY_ID = "china-policy";
export const POLICY_COMPANY = { id: POLICY_COMPANY_ID, name_ko: "중국 배터리·NEV·ESS 정책", name_zh: "中国电池新能源汽车储能政策", name_en: "China battery NEV ESS policy", type_tags: ["policy"], aliases: [] };
...
export function buildSearchGroups(provider, pilot = false, bootstrapCompanyIds = [], companies = TRACKED_COMPANIES) {
  const size = SEARCH_GROUP_SIZE[searchLaneEngine(provider)] || 4;
...
export function plannedSearchRequests(pilot = false, bootstrapCompanyIds = [], companies = TRACKED_COMPANIES) {
  const companyRequests = SEARCH_LANES
    .filter((lane) => llmConfig(searchLaneEngine(lane)))
    .reduce((total, lane) => total + buildSearchGroups(lane, pilot, bootstrapCompanyIds, companies).length, 0);
  const policyRequests = pilot ? 0 : policySearchEngines().length;
  return companyRequests + policyRequests;
}

export function publishedDayInWindow(value, since, until) {
  const day = String(value || "").slice(0, 10);
  return /^\d{4}-\d{2}-\d{2}$/.test(day) && day >= since && day <= until;
}


// 검색 프롬프트. 관리자 화면(파이프라인 보기)이 같은 함수를 불러 실제 값을 보여 준다.
// 화면에 옮겨 적으면 코드와 어긋나므로 정의를 한 곳에 두고 양쪽이 같이 읽는다.
export function searchProviderPrompt(lane) {
  if (lane === "china_local" || lane === "deepseek") {
    // 2026-09-10 DeepSeek 검색이 끊긴 뒤 당일 발행 중국 기사 발견이 실행당 8~23건에서 0건으로 떨어졌다
    // (운영 DB, 09-14 확인). OpenAI 웹 검색은 두 레인 모두 1~3일 지난 포털 재게재 기사만 돌려줬다.
    // 그래서 오늘·어제를 먼저 찾게 하고, 전문매체는 이름이 아니라 도메인을 주어 site: 검색으로 직접 뒤지게 한다.
    const base = "당신은 중국 현지 배터리 산업 리서처다. 중국어 검색어로 찾고 중국어 원문을 우선한다. 발행일이 오늘·어제인 기사를 먼저 찾는다. 검색어에 회사 중문명과 함께 '最新' 또는 오늘·어제 날짜(예: 9月14日)를 넣고, 산업 전문매체를 site: 검색으로 직접 뒤진다: 高工锂电 gg-lb.com, 电池中国 cbea.com, OFweek锂电 libattery.ofweek.com, 电池网 itdcw.com, 中国储能网 escn.com.cn, 第一电动 d1ev.com, 盖世汽车 gasgoo.com. 성·시 지방정부와 산업단지 공고, 기업 공식 발표·투자자 관계 페이지도 글로벌 매체보다 먼저 본다. 같은 소식이면 며칠 지난 포털 재게재보다 전문매체 원문을 택한다. 지금까지 실제 확인한 기사만 JSON으로 반환하며, 목표 건수는 상한일 뿐이라 0건이어도 그대로 반환한다. 찾지 못한 기사는 만들지 않는다.";
    // DeepSeek 엔진은 목표 건수를 채우려 검색을 반복하다 최종 JSON을 못 내던 이력이 있어 호출 횟수를 묶어 둔다.
    return searchLaneEngine(lane) === "deepseek"
      ? `${base} 반드시 웹 검색 도구를 한 번 이상 실행한다. 회사마다 검색 1회씩만 하고, 필요한 근거를 찾으면 검색을 끝내 최종 JSON을 반환한다. 그 결과에 기사가 없으면 그 회사는 건너뛰며, 같은 회사를 다른 검색어로 다시 찾지 않는다.`
      : base;
  }
  return "당신은 글로벌 배터리 산업 리서처다. 공식 발표와 신뢰 가능한 주요 언론을 우선해, 기업의 사업·생산·고객·기술·재무·해외 전략 변화를 폭넓게 탐색한다. 발행일이 오늘·어제인 기사를 먼저 찾고, 검색어에 '最新'·'latest'나 오늘 날짜를 넣어 최신 보도를 끌어낸다.";
}
export function searchInstructions(providerInstructions) {
  return `중국 이차전지 산업 뉴스 리서처다. 웹 검색을 이용한다. ${providerInstructions} 검색 결과에 실제로 제시된 원문 기사 URL만 반환한다. URL·제목·매체·날짜를 추정하거나 만들어내지 않는다. 특히 원문에 월·일만 보인다는 이유로 현재 연도를 붙이지 않는다. 검색 결과의 상대 시각이나 크롤링 시각을 발행일로 쓰지 말고, 원문 본문 또는 구조화된 발행 메타데이터에서 연·월·일을 직접 확인한다. 확인한 발행일이 요청 기간 밖이면 반환하지 않으며, 연도를 확인할 수 없으면 published_at을 null로 반환한다. 광고, 자동차 소비자 리뷰, 주가 단신은 제외한다. 중국어/영어 원문 모두 가능하다. 대괄호 안은 서비스의 한국어 표준명과 중국어·영어 검색 별칭이다. 그룹 회사는 모회사뿐 아니라 명시된 주요 계열사 관련 기사도 찾되, 관계가 불명확한 동명사는 포함하지 않는다. 회사 목록은 탐색 범위이며 모든 회사의 기사를 반드시 찾을 필요는 없다. 실제 발견한 기사만 반환한다.`;
}

async function discoverWebSearchNews(lane, pilot = false, bootstrapCompanyIds = [], companies = TRACKED_COMPANIES) {
  // provider 필드는 레인 이름이다(article.discovered_via·source_tier에 web_search_<레인>으로 남는다).
  // 실제로 호출한 LLM은 engine에 따로 적어 어느 검색 도구가 찾았는지 화면에서 구분한다.
  const provider = lane;
  const engine = searchLaneEngine(lane);
  if (!llmConfig(engine)) {
    // 키가 없어 아예 부르지 않은 것과, 불렀는데 결과가 없는 것은 원인이 전혀 다르다.
    webSearchOutcomes.push({ provider, engine, group: "all", articles: 0, error: "NOT_CONFIGURED" });
    return [];
  }
  const groups = buildSearchGroups(lane, pilot, bootstrapCompanyIds, companies);
  ...
  const articleCap = (companyCount) => engine === "deepseek"
  ...
  const results = await Promise.all(groups.map(async (group) => {
    ...
    try {
      ...
      const result = await createJsonResponse({
        ...
        input: `${since}부터 ${until}까지 ${group.label} 그룹/기업(${names})의 사업·생산·증설·수주·고객·기술·재무·해외 전략과 직접 관련된 기사 후보를 최대 ${maxItems}건 찾으세요. 가장 최근 발행 기사를 먼저 찾으세요(오늘 ${until}·어제 우선, 그다음 ${since}까지). 목표 건수는 상한이며 적게 찾았거나 없으면 그대로 반환하세요. 각 기사에는 제목, 원문 URL, 원 매체명, 발행일(모르면 null), 검색 결과에 보이는 1~2문장 요약을 반환하세요.`,
        provider: engine
      });
      webSearchOutcomes.push({ provider, engine, group: group.label, articles: (result.data.articles || []).length, telemetry: result.telemetry });
      return { articles: result.data.articles || [], sourceUrls: result.sourceUrls, provider, bootstrap: Boolean(group.bootstrap) };
    } catch (error) {
      console.error("[WEB_SEARCH_FAILED]", JSON.stringify({ provider, engine, message: error.message }));
      // 실패를 여기서 삼키고 빈 배열만 돌려주면 호출한 쪽은 "검색은 됐는데 결과가 없다"와
      // 구분하지 못한다. 발견 0건의 원인을 화면에서 볼 수 있도록 사유를 남긴다.
      webSearchOutcomes.push({ provider, engine, group: group.label, articles: 0, error: String(error.message || error).slice(0, 300) });
  ...
  return results.flatMap(({ articles, sourceUrls, provider, bootstrap }) => articles.map((item) => ({ ...item, sourceUrls, provider, bootstrap }))).map((item) => {
    try {
      ...
      const windowDays = item.bootstrap ? BOOTSTRAP_WINDOW_DAYS : 3;
      const until = koreaDate();
      const since = koreaDate(new Date(Date.now() - windowDays * 86400000));
      if (!publishedDayInWindow(item.published_at, since, until)) return null;
      return { source: item.source_name || "Web news", title: clean(item.title), url: url.toString(), publishedAt: item.published_at, snippet: clean(item.snippet), kind: "web_search_news", searchProvider: item.provider, bootstrap: Boolean(item.bootstrap) };
    } catch { return null; }
  }).filter(Boolean);
}

async function discoverPolicyNews(provider) {
  if (!llmConfig(provider)) return [];
  const until = koreaDate();
  const since = koreaDate(new Date(Date.now() - 3 * 86400000));
  try {
    const result = await createJsonResponse({
      name: "china_battery_policy_candidates",
      provider,
      webSearch: true,
      instructions: "중국 중앙정부의 배터리·신에너지차(NEV)·신형 에너지저장(ESS) 정책 리서처다. 국무원과 공업정보화부·발전개혁위원회·국가에너지국·상무부·재정부·세무총국·시장감독관리총국 등 정부 원문을 최우선으로 찾는다. 법령·공고·의견·표준·세제·보조금·수출통제·산업규범·안전·재활용 정책의 신규 발표나 시행 변화만 반환한다. 기업 뉴스, 전망 기사, 과거 정책을 새 기사처럼 재소개한 글은 제외하고 실제 URL을 만들지 않는다.",
      input: `${since}부터 ${until}까지 새로 발표되었거나 시행 상태가 바뀐 중국의 배터리·NEV·ESS 정책을 최대 6건 찾으세요. 각 항목에 title, url, source_name, published_at, snippet을 반환하세요. 없으면 빈 배열로 반환하세요.`,
      schema: { type: "object", additionalProperties: false, required: ["articles"], properties: { articles: { type: "array", maxItems: 6, items: { type: "object", additionalProperties: false, required: ["title", "url", "source_name", "published_at", "snippet"], properties: { title: { type: "string" }, url: { type: "string" }, source_name: { type: "string" }, published_at: { type: ["string", "null"] }, snippet: { type: "string" } } } } } }
    });
    webSearchOutcomes.push({ provider, engine: provider, group: "중국 정책", articles: (result.data.articles || []).length, telemetry: result.telemetry });
    return (result.data.articles || []).filter(item => {
      try {
        const url = new URL(item.url);
        const day = String(item.published_at || '').slice(0, 10);
        return /^https?:$/.test(url.protocol) && /^\d{4}-\d{2}-\d{2}$/.test(day) && day >= since && day <= until
          && (!result.sourceUrls?.length || result.sourceUrls.some(source => { try { return new URL(source).hostname === url.hostname; } catch { return false; } }));
      } catch { return false; }
    }).map(item => ({ source: item.source_name || "중국 정부", title: clean(item.title), url: item.url, publishedAt: item.published_at, snippet: clean(item.snippet), kind: "policy_news", searchProvider: provider, companyId: POLICY_COMPANY_ID }));
  } catch (error) {
    webSearchOutcomes.push({ provider, engine: provider, group: "중국 정책", articles: 0, error: String(error.message || error).slice(0, 300) });
    return [];
  }
}
...
export function discoveredVia(item) {
  if (item.kind === "policy_news") return `web_search_policy_${item.searchProvider || 'discovered'}`;
...
export async function discoverChinaSources({pilot = false, bootstrapCompanyIds = []} = {}) {
  ...
  const policyEngines = pilot ? [] : policySearchEngines();
  const labels = [...SEARCH_LANES.map((lane) => `web_search_${lane}`), ...policyEngines.map((engine) => `policy_${engine}`), "catl_newsroom", ...selected.map((company) => `cninfo:${company.id}`)];
  const collectors = [...SEARCH_LANES.map((lane) => discoverWebSearchNews(lane, pilot, bootstrapCompanyIds, companies)), ...policyEngines.map((engine) => discoverPolicyNews(engine)), fetchCatlNewsroom(), ...selected.map(fetchCninfo)];
  ...
  for (const item of items) {
    ...
    if (item.kind === 'policy_news') { prev.kind = item.kind; prev.companyId = POLICY_COMPANY_ID; }
  }
...
export function companiesFor(item, companies = TRACKED_COMPANIES) {
  if (item.companyId === POLICY_COMPANY_ID) return [POLICY_COMPANY];
