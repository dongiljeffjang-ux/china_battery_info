...
async function discoverWebSearchNews(provider, pilot = false) {
  ...
  const providerInstructions = provider === "deepseek"
    ? "당신은 중국 현지 배터리 산업 리서처다. 중국어 원문과 중국 산업 전문매체·지역 정부·기업 발표를 우선한다. 회사마다 검색 1회씩만 하고, 그 결과에 기사가 없으면 그 회사는 건너뛴다. 같은 회사를 다른 검색어로 다시 찾지 않는다. 지금까지 실제 확인한 기사만 JSON으로 반환하며, 목표 건수는 상한일 뿐이라 0건이어도 그대로 반환한다. 찾지 못한 기사는 만들지 않는다."
