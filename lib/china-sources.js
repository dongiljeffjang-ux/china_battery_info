...
export function plannedSearchRequests(pilot = false, bootstrapCompanyIds = [], companies = TRACKED_COMPANIES) {
  const companyRequests = ["openai", "deepseek"]
  ...
  const policyRequests = pilot ? 0 : ["openai", "deepseek"].filter((provider) => llmConfig(provider)).length;
...
export function searchProviderPrompt(provider) {
  return provider === "deepseek" ? "당신은 중국 현지 배터리 산업 리서처다. 반드시 웹 검색 도구를 한 번 이상 실행한다. 중국어 원문과 중국 산업 전문매체·지역 정부·기업 발표를 우선한다. 회사마다 검색 1회씩만 하고, 필요한 근거를 찾으면 검색을 끝내 최종 JSON을 반환한다. 그 결과에 기사가 없으면 그 회사는 건너뛰며, 같은 회사를 다른 검색어로 다시 찾지 않는다. 지금까지 실제 확인한 기사만 JSON으로 반환하며, 목표 건수는 상한일 뿐이라 0건이어도 그대로 반환한다. 찾지 못한 기사는 만들지 않는다." : "당신은 글로벌 배터리 산업 리서처다. 공식 발표와 신뢰 가능한 주요 언론을 우선해, 기업의 사업·생산·고객·기술·재무·해외 전략 변화를 폭넓게 탐색한다.";
}
...
async function discoverPolicyNews(provider) {
  ...
  try {
    ...
    webSearchOutcomes.push({ provider, group: "중국 정책", articles: (result.data.articles || []).length, telemetry: result.telemetry });
    ...
    webSearchOutcomes.push({ provider, group: "중국 정책", articles: 0, error: String(error.message || error).slice(0, 300) });
    return [];
  }
}
...
export async function discoverChinaSources({pilot = false, bootstrapCompanyIds = []} = {}) {
  ...
  const policyCollectors = pilot ? [] : [discoverPolicyNews("openai"), discoverPolicyNews("deepseek")];
  const labels = ["web_search_openai", "web_search_deepseek", ...(pilot ? [] : ["policy_openai", "policy_deepseek"]), "catl_newsroom", ...selected.map((company) => `cninfo:${company.id}`)];
  const collectors = [discoverWebSearchNews("openai",pilot,bootstrapCompanyIds,companies), discoverWebSearchNews("deepseek",pilot,bootstrapCompanyIds,companies), ...policyCollectors, fetchCatlNewsroom(), ...selected.map(fetchCninfo)];
