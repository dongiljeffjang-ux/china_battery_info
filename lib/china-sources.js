...
async function discoverDeepSeekNews() {
  if (llmConfig()?.provider !== "deepseek") return [];
  ...
  return results.flat().map((item) => {
    try {
      ...
      return { source: item.source_name || "Web news", title: clean(item.title), url: url.toString(), publishedAt: item.published_at || new Date().toISOString(), snippet: clean(item.snippet), kind: "deepseek_news" };
    } catch { return null; }
  }).filter(Boolean);
}
...
export async function discoverChinaSources() {
  const [deepSeekNews, catlNews, ...disclosures] = await Promise.all([discoverDeepSeekNews(), fetchCatlNewsroom(), ...COMPANIES.map(fetchCninfo)]);
  return [...new Map([...deepSeekNews, ...catlNews, ...disclosures.flat()].map((item) => [item.url, item])).values()];
}
