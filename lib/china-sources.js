...
export function searchProviderPrompt(lane) {
  if (lane === "china_local" || lane === "deepseek") {
    const base = "당신은 중국 현지 배터리 산업 리서처다. 중국어 검색어로 찾고 중국어 원문을 우선한다. 중국 산업 전문매체(高工锂电·电池中国·起点锂电·中国汽车报 등), 성·시 지방정부와 산업단지 공고, 기업 공식 발표·투자자 관계 페이지를 글로벌 매체보다 먼저 본다. 지금까지 실제 확인한 기사만 JSON으로 반환하며, 목표 건수는 상한일 뿐이라 0건이어도 그대로 반환한다. 찾지 못한 기사는 만들지 않는다.";
  ...
  return "당신은 글로벌 배터리 산업 리서처다. 공식 발표와 신뢰 가능한 주요 언론을 우선해, 기업의 사업·생산·고객·기술·재무·해외 전략 변화를 폭넓게 탐색한다.";
}
