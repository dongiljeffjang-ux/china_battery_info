...
async function discoverWebSearchNews(provider) {
  ...
  const articleCap = (companyCount) => Math.min(18, Math.max(12, Math.round(companyCount * 1.5)));
  ...
  const results = await Promise.all(groups.map(async (group) => {
    try {
      ...
      webSearchOutcomes.push({ provider, group: group.label, articles: (result.data.articles || []).length });
...
export async function discoverChinaSources() {
  webSearchOutcomes = [];
  const labels = ["web_search_openai", "web_search_deepseek", "catl_newsroom", ...COMPANIES.filter((company) => company.cninfo).map((company) => `cninfo:${company.id}`)];
