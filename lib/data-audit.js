...
function corpus(article) {
  ...
  return [article.title_original, article.title_ko, article.summary_ko, ...keywords]
    .filter(Boolean).join(" \n");
}
...
function detectedCompanyIds(article, companies) {
  const text = corpus(article).toLowerCase();
...
export function buildDataAudit({ articles = [], chunks = [], events = [], companies = COMPANIES } = {}) {
  ...
  for (const article of articles) {
    ...
    const detected = detectedCompanyIds(article, companies);
    if (!linked.length) {
    ...
    for (const companyId of linked) {
      ...
      issues.push({
        ...
        severity: otherDetected.length && !official ? "high" : "review",
