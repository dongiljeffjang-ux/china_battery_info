...
async function pendingWork() {
  ...
  const [facts, concepts, basis, article, headline, report] = await Promise.all([
  ...
  return { facts, concepts, basis, article, headline, report };
}

// 무거운 단계는 한 훅에 하나만 맡는다.
//
// 예전에는 한 훅이 모든 단계를 차례로 시도했다. LLM 호출 하나가 25~40초라 두세 단계만 돌아도
// 60초 함수 한도를 넘어 함수가 죽고, 다음 훅 호출이 나가지 못해 체인이 끊겼다(2026-09-08:
// 훅이 82초·97초를 쓰고 2~4훅에서 멈춤). 훅마다 하나씩 돌아가며 맡으면 한 훅이 한 호출이라
...
export async function runCurationHop({ deadline, hop = 1 }) {
  ...
  // 보고서 읽기·보강·웹 백필은 예산이 남을 때만 손댄다. 하나만 하고 끝낸다.
  let due = null;
  let thinLeft = false;
  ...
  // 보강 패스. 얇게 읽힌 보고서를 다시 읽는다. 한 훅에 한 건만.
  if (!due && !web && Date.now() < deadline) {
    let thin = null;
    try {
      thin = await pickThinReport();
      if (thin) { log.enrich = [await enrichReport(thin)]; thinLeft = true; }
    } catch (error) {
      log.enrich = [{ error: error.message, company_id: thin?.company_id, title: thin?.report_title }];
      thinLeft = true;
  ...
  log.pending = pending;
  const more = Boolean(due) || Boolean(web) || thinLeft
