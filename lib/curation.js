...
export async function digestCompany(company, kind, window) {
  ...
  const { rows, dropped, returned, report } = await digestReport({ company, kind, knownUrl: found.url, maxEvents: DIGEST_MAX_EVENTS, timeoutMs: REPORT_LLM_TIMEOUT_MS });
...
export async function enrichReport(row) {
  ...
  const { rows, dropped, returned, report } = await digestReport({ company, kind: row.kind, knownUrl: row.report_url, maxEvents: DIGEST_MAX_EVENTS, timeoutMs: REPORT_LLM_TIMEOUT_MS });
...
export async function renewReport(row) {
  ...
  const canReplace = rows.length >= Math.max(RENEW_REPLACE_MIN_EVENTS, Math.ceil(oldCount * RENEW_REPLACE_MIN_RATIO));
  let replaced = 0;
  if (canReplace) {
