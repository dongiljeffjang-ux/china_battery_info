...
export async function digestCompany(company, kind, window) {
  const candidates = await findReports(company, kind, { years: COVERAGE_YEARS + 0.7 });
  const found = window
...
export async function runCurationHop({ deadline }) {
  ...
  // 구조화 사실 추출. 새 이벤트는 매일 조금씩, 처음에는 밀린 것을 훅마다 몇 묶음씩 처리한다.
  ...
  const more = Boolean(due) || Boolean(web) || Boolean(redate && redate.remaining > 0) || Boolean(log.embed && log.embed.remaining > 0) || Boolean(log.facts && log.facts.remaining > 0);
  return { log, more };
}
