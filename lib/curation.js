...
// 예전에 읽은 보고서는 본문이 비어 있으므로 그때만 PDF를 다시 내려받아 채운다.
export async function embedMissingReports(limit = REPORT_EMBED_BATCH) {
  const done = await supabaseRest("knowledge_chunk?select=source_url&source_type=eq.report_chunk");
  const embeddedUrls = new Set((done || []).map((row) => row.source_url));
  const ledger = await supabaseRest(
    "report_digest?select=company_id,kind,report_url,report_title,published_at,body_section,body_chars"
    + "&kind=in.(annual,semiannual)&order=published_at.desc&limit=400"
  );
  const pending = (ledger || []).filter((row) =>
    !row.report_url.startsWith("missing:") && !row.report_url.startsWith("web:") && !embeddedUrls.has(row.report_url));
  ...
  for (const row of pending.slice(0, limit)) {
    ...
    try {
      ...
      const result = await embedReportChunks({
        ...
        reportUrl: row.report_url, reportTitle: row.report_title, publishedAt: row.published_at, section,
      });
      embedded += result.chunks;
      reports += 1;
    } catch (error) {
...
export async function runCurationHop({ deadline }) {
  ...
  try { log.embed_report = await embedMissingReports(); } catch (error) { log.embed_report = { error: error.message }; }
  if (Date.now() > deadline) return { log, more: true };
  ...
  const more = Boolean(due) || Boolean(web) || thinLeft || Boolean(redate && redate.remaining > 0)
    ...
    || Boolean(log.embed_report && log.embed_report.remaining > 0)
