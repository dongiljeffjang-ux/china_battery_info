...
export async function embedMissingArticles(limit = ARTICLE_EMBED_BATCH) {
  const rows = await supabaseRest(
    ...
    + "&verification_status=in.(verified,pending_review)&embedding_status=neq.embedded&body_original=not.is.null"
...
async function pendingWork() {
  ...
  const [facts, concepts, basis, article, headline, report, digest, web, enrich, renew] = await Promise.all([
    ...
    has(`article?select=id&verification_status=in.(verified,pending_review)&embedding_status=neq.embedded&body_original=not.is.null&embedding_attempts=lt.${ARTICLE_EMBED_MAX_ATTEMPTS}&limit=1`),
