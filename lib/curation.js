...
export async function pickDueWebBackfill(now = new Date()) {
  ...
  const years = [];
  for (let year = Number(WEB_BACKFILL_SINCE.slice(0, 4)); year <= now.getUTCFullYear(); year += 1) years.push(year);
  for (const company of COMPANIES) {
    if (company.cninfo || company.hkex) continue;
...
export async function runCurationHop({ deadline }) {
  ...
  const more = Boolean(due) || Boolean(web) || thinLeft || Boolean(redate && redate.remaining > 0) || Boolean(log.embed && log.embed.remaining > 0);
  return { log, more };
}
