...
export async function runCurationHop({ deadline }) {
  ...
  // 사업 관계(회사 ↔ 고객·공급사·정부) 추출. 지금까지 훅에 연결돼 있지 않아 이벤트 585건 중
  // 10건만 추출돼 있었다. 관계망 화면의 재료라 임베딩 다음으로 앞에 둬 예산을 먼저 받게 한다.
  try { log.facts = await extractMissingFacts({ deadline }); } catch (error) { log.facts = { error: error.message }; }
  if (Date.now() > deadline) return { log, more: true };
  ...
  // 기사 본문이 시점을 적지 않아 발행일이 들어간 이벤트를 웹 검색으로 재확인한다. 회사 하나, 20건씩.
  try { log.redate_article = await redateMissingArticleEvents(); } catch (error) { log.redate_article = { error: error.message }; }
  if (Date.now() > deadline) return { log, more: true };
  ...
  const more = Boolean(due) || Boolean(web) || thinLeft || Boolean(redate && redate.remaining > 0)
    ...
    || Boolean(log.facts && log.facts.remaining > 0)
    || Boolean(log.redate_article && log.redate_article.remaining > 0);
  return { log, more };
}
