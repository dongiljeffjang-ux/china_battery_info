...
export async function digestReport({ company, kind = "annual", knownUrl = null, maxEvents = 10, provider = "auto", timeoutMs }) {
  ...
  return { rows, dropped, returned: (result.data.events || []).length, provider: result.provider, report: { url: report.url, title: report.title, published_at: report.published_at, pages: report.pages, text: report.text, section: report.section, parse_quality: report.parse_quality, visual_pages: report.visual_pages, source_sha256: report.source_sha256 } };
}
