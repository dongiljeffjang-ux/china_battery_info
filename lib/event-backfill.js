// 과거 기업 시계열 백필.
//
// 회사 1곳당 웹 검색 1회로 과거 주요 사실을 받아 event 행 후보를 만든다.
// 본문 정독·교차검증을 거치지 않으므로 timeline_eligibility는 reference로 고정한다.
// 나중에 공시로 확인되면 core로 승격한다.
//
// api/ingest-rss.js(Vercel 키 사용)와 scripts/backfill-events.mjs(로컬)가 함께 쓴다.

import crypto from "node:crypto";
import { createJsonResponse } from "./llm-provider.js";
import { groupSearchEntities, matchGroupEntities } from "./company-groups.js";
import { LAYER_ENUM, LAYER_PROMPT_GUIDE, normalizeLayerKey } from "./timeline-layers.js";
import { REPORT_KINDS, readReport } from "./report-reader.js";

function eventSchema(maxEvents) {
  return {
    type: "object",
    additionalProperties: false,
    required: ["events"],
    properties: {
      events: {
        type: "array",
        maxItems: maxEvents,
        items: {
          type: "object",
          additionalProperties: false,
          required: ["occurred_at", "occurred_precision", "occurred_basis", "title_ko", "fact_ko", "trajectory_track", "layer_key", "region_scope", "source_url", "source_name", "original_excerpt", "original_excerpt_ko"],
          properties: {
            occurred_at: { type: "string" },
            occurred_precision: { type: "string", enum: ["day", "month", "half", "year"] },
            occurred_basis: { type: "string" },
            title_ko: { type: "string" },
            fact_ko: { type: "string" },
            trajectory_track: { type: "string", enum: ["market", "technology"] },
            layer_key: { type: "string", enum: LAYER_ENUM },
            region_scope: { type: ["string", "null"] },
            source_url: { type: "string" },
            source_name: { type: "string" },
            original_excerpt: { type: "string" },
            original_excerpt_ko: { type: "string" },
          },
        },
      },
    },
  };
}

const INSTRUCTIONS = [
  "중국 배터리 산업 리서처다. 웹 검색으로 특정 회사의 과거 주요 사실을 모은다.",
  "검색 결과에 실제로 제시된 원문 기사·공시 URL만 반환한다. URL·제목·매체·날짜를 추정하거나 만들어내지 않는다.",
  "출처에 명시된 사실만 한국어로 쓴다. 전망·인과 추정·성공 가능성·투자 판단을 만들지 않는다.",
  // 2026-09-07 Reshine 수동 백필에서 모델이 title_ko·fact_ko·original_excerpt_ko를 전부 중국어
  // 원문 그대로 돌려줬다(12건 전부). 한국어 지시가 목록 가운데 한 번뿐이라 묻힌 것으로 보고,
  // 필드 이름을 짚어 다시 못박는다. 서버도 한글이 없는 건은 버린다(아래 dropped.notKorean).
  "title_ko·fact_ko·original_excerpt_ko는 반드시 한국어 문장으로 쓴다. 중국어·영어 원문을 그대로 옮겨 적으면 그 건은 버려진다. 회사명·제품명·지명은 한국어 표기 뒤 괄호로 원문을 병기할 수 있지만, 문장 자체는 한국어여야 한다. 원문 그대로 남기는 필드는 original_excerpt 하나뿐이다.",
  "occurred_at은 사건이 일어난 날짜를 YYYY-MM-DD로 쓴다. 기사 발행일이 아니라 본문이 말하는 사건일을 우선한다. 날짜가 불명확한 항목은 반환하지 않는다.",
  "occurred_precision은 그 날짜를 어디까지 믿을 수 있는지다. 날짜까지 확인되면 day, 월까지면 month, 반기까지면 half, 연간 집계면 year를 쓴다.",
  "occurred_basis에는 그 날짜를 그렇게 정한 근거를 한 문장으로 쓴다.",
  "original_excerpt에는 근거가 되는 원문을 300자 이내로 발췌하고, original_excerpt_ko에는 그 발췌문의 충실한 한국어 번역만 쓴다.",
  // 보고서 추출과 같은 규칙. 한 건에 출처 여기저기의 수치를 모으면 발췌가 그 건을 뒷받침하지 못한다.
  "한 건에는 같은 문단에서 나온 수치만 담는다. 서로 다른 대목의 수치를 한 건으로 모으지 않고 별개 건으로 나눈다. fact_ko에 쓴 수치는 하나도 빠짐없이 그 건의 original_excerpt 안에 있어야 한다.",
  "본문에 마크다운 링크나 인용 표기를 넣지 않는다. 출처는 source_url 필드에만 쓴다.",
  "같은 사건을 중복해 넣지 않는다. 시기가 고르게 분포하도록 서로 다른 분기의 사실을 우선한다.",
  "주가 단신, 애널리스트 목표주가, 자동차 소비자 리뷰, 광고는 제외한다.",
  LAYER_PROMPT_GUIDE,
].join(" ");

// 모델이 사실 문장 끝에 ([catl.com](https://...)) 같은 인용을 붙인다.
// 출처는 source_url로 따로 저장하므로 본문에서는 걷어낸다.
export function stripCitations(value) {
  return String(value || "")
    .replace(/\s*\(\s*\[[^\]]*\]\([^)]*\)\s*\)/g, "")
    .replace(/\[([^\]]*)\]\([^)]*\)/g, "$1")
    .replace(/\s+/g, " ")
    .trim();
}

function cleanUrl(value) {
  try {
    const url = new URL(value);
    url.searchParams.delete("utm_source");
    return url.toString();
  } catch {
    return value;
  }
}

// layer_key가 정해지면 그 소속으로 트랙을 맞춘다. 시장 레이어인데 트랙이 기술로 들어가는
// 어긋남을 막고, 화면이 이벤트를 한 열에만 놓을 수 있게 한다(both는 쓰지 않는다).
function trackFor(layerKey, fallback) {
  if (layerKey) return layerKey.startsWith("technology-") ? "technology" : "market";
  return fallback === "technology" ? "technology" : "market";
}

function hostnameOf(value) {
  try {
    const url = new URL(value);
    return /^https?:$/.test(url.protocol) ? url.hostname : null;
  } catch {
    return null;
  }
}

// 화면에 뜨는 문장이 한국어인지 서버가 확인한다. 모델에게 맡기지 않는다.
//
// 2026-09-07 Reshine 백필에서 모델이 title_ko·fact_ko를 중국어 원문 그대로 돌려줬고(12건 전부),
// 그대로 저장돼 한국어 화면에 중국어 문장이 실렸다. 근거 등급을 서버가 정하는 것과 같은 이유로
// 표시 언어도 서버가 지킨다. 한글이 한 글자도 없으면 번역을 안 한 것으로 보고 그 건을 버린다.
// original_excerpt(원문 발췌)는 중국어가 정상이므로 검사하지 않는다.
export function hasKorean(value) {
  return /[가-힣]/.test(String(value || ""));
}

// 보고서 추출 결과는 모델이 낸 요약이므로, 원문 발췌만큼은 서버가 입력 조각에서 다시 확인한다.
// 표는 좌표 기반으로 한 행씩 복원한 뒤 모델에 전달한다. 그럼에도 모델이 표의 다른 행을 섞거나
// 존재하지 않는 발췌를 만들면 이 검사를 통과하지 못해 저장 후보가 되지 않는다.
// 공백·표 셀 구분자는 PDF 추출/JSON 직렬화 과정에서 달라질 수 있어 비교에서만 없앤다. 숫자와
// 문자는 그대로 비교하므로 숫자 하나를 지어낸 발췌는 통과할 수 없다.
export function isGroundedReportExcerpt(excerpt, sourceChunk) {
  const normalize = (value) => String(value || "")
    .normalize("NFKC")
    .replace(/[\s|]/g, "")
    .replace(/[“”\"'‘’]/g, "")
    .trim();
  const candidate = normalize(stripCitations(excerpt));
  return candidate.length >= 8 && normalize(sourceChunk).includes(candidate);
}

export async function backfillCompanyEvents({ company, since, until, maxEvents = 20, provider = "auto", timeoutMs }) {
  const entities = groupSearchEntities(company.id);
  const names = [company.name_zh, company.name_en].filter(Boolean).join(" / ");
  const groupLine = entities.length ? `\n같은 그룹의 주요 계열사도 함께 찾는다: ${entities.join(" / ")}` : "";
  const result = await createJsonResponse({
    name: "company_timeline_backfill",
    schema: eventSchema(maxEvents),
    webSearch: true,
    instructions: INSTRUCTIONS,
    input: `회사: ${company.name_ko} [${names}]${groupLine}\n기간: ${since} ~ ${until}\n이 기간에 이 회사(또는 명시된 계열사)에서 실제로 일어난 주요 사실을 최대 ${maxEvents}건 찾으세요. 증설·가동·생산능력, 고객 인증·수주·공급계약, 제품·기술·특허, 실적·가격, 해외 진출·현지 법인을 우선합니다.`,
    provider,
    timeoutMs,
  });

  const citations = new Set((result.sourceUrls || []).map(hostnameOf).filter(Boolean));
  const rows = [];
  const dropped = { badDate: 0, badUrl: 0, uncited: 0, notKorean: 0 };
  for (const event of result.data.events || []) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(event.occurred_at) || event.occurred_at < since || event.occurred_at > until) {
      dropped.badDate += 1;
      continue;
    }
    const host = hostnameOf(event.source_url);
    if (!host) {
      dropped.badUrl += 1;
      continue;
    }
    // 검색이 실제로 인용한 도메인만 채택한다. 모델이 URL을 지어내는 것을 막는 장치다.
    if (citations.size && !citations.has(host)) {
      dropped.uncited += 1;
      continue;
    }
    const title = stripCitations(event.title_ko);
    const fact = stripCitations(event.fact_ko);
    if (!title || !fact) {
      dropped.badUrl += 1;
      continue;
    }
    // 번역을 건너뛴 건은 한국어 화면에 중국어 문장으로 실린다. 저장 전에 버린다.
    if (!hasKorean(title) || !hasKorean(fact)) {
      dropped.notKorean += 1;
      continue;
    }
    rows.push({
      company_id: company.id,
      occurred_at: event.occurred_at,
      occurred_precision: event.occurred_precision || "day",
      occurred_basis: event.occurred_basis || null,
      title_ko: title,
      fact_ko: fact,
      trajectory_track: trackFor(normalizeLayerKey(event.layer_key), event.trajectory_track),
      layer_key: normalizeLayerKey(event.layer_key),
      region_scope: event.region_scope || null,
      source_url: cleanUrl(event.source_url),
      source_name: event.source_name,
      original_excerpt: stripCitations(event.original_excerpt),
      original_excerpt_ko: stripCitations(event.original_excerpt_ko),
      timeline_eligibility: "reference",
      evidence_kind: "web_backfill",
      entity_names: matchGroupEntities(company.id, `${title} ${fact} ${event.original_excerpt}`),
    });
  }
  return { rows, dropped, returned: (result.data.events || []).length, provider: result.provider };
}

// 정기보고서 원문을 읽어 핵심 사실을 간추린다.
// 웹 검색을 쓰지 않으므로 모델이 URL을 지어낼 수 없고, 출처는 항상 그 보고서다.
const DIGEST_INSTRUCTIONS = [
  "중국 배터리 산업 애널리스트다. 제공된 거래소 정기보고서 원문에서 이 회사(또는 명시된 계열사)에 관한 사실을 빠짐없이 뽑는다.",
  "원문에 적힌 사실만 쓴다. 원문에 없는 수치·연도·회사명을 만들지 않는다. 전망·목표·전략 서술은 넣지 않는다.",
  "수치가 붙은 사실은 빠짐없이 뽑는다: 생산능력(기지별·품목별), 출하량·판매량, 가동률, 증설·프로젝트 투자액과 진척, 주요 고객·수주·공급계약(상대방 이름), 신제품 스펙과 양산 단계, 인증, 매출·이익·부문별 매출, 해외 법인·공장, 특허 수, 모집자금 프로젝트의 건설·투입·완공 진척, 대형 계약. 단, 모집자금 전용계좌 잔액·보증·부채·채권·특수관계자 거래·감사·배당처럼 사업 변화를 설명하지 않는 공시 관리 항목은 뽑지 않는다.",
  "수치가 없는 서술은 사업 궤적을 바꾸는 것(신규 고객 확보, 신규 거점, 신제품 양산 개시, 인증 획득, 합작·인수)만 남긴다. 회사 소개, 업계 통계, 정책 인용, 리스크 서술은 넣지 않는다.",
  "같은 사실을 표현만 바꿔 여러 건으로 만들지 않는다. 한 문장에 기지별 수치가 여럿이면 기지마다 한 건으로 나눈다.",
  // 한 건에 원문 여기저기의 수치를 모으면 그 건의 발췌로는 일부밖에 뒷받침하지 못한다.
  // 2026-09-07 점검에서 수치 7개 이상을 담은 건의 91%가 발췌에 없는 수치를 갖고 있었다.
  "한 건에는 원문의 같은 문장, 또는 같은 표의 같은 행에서 나온 수치만 담는다. 다른 문단·다른 절·다른 표에 있는 수치를 한 건으로 모으지 않는다. 실적, 프로젝트 진척, 생산능력, 재무 항목은 원문에서 서로 떨어져 있으므로 각각 별개 건으로 낸다.",
  // 표의 금액은 단위가 표 머리(单位：万元 / 千元 / 元)에만 적혀 있어 행만 보면 알 수 없다.
  // 2026-09-07 Farasis 반기보고서 보증 표에서 万元을 千元로 읽어 5건이 10배 작게 저장됐다.
  // 원문 표기를 그대로 남겨야 사람이든 기계든 환산을 다시 검산할 수 있다.
  "표에서 금액을 읽을 때는 그 표의 단위 표기(单位：元 / 千元 / 万元 / 百万元)를 표 머리에서 반드시 확인하고 그 단위로 환산한다. 단위를 확인하지 못했으면 환산하지 말고 원문 숫자와 '단위 미확인'을 그대로 쓴다. 금액을 한국어로 옮길 때는 반드시 원문 표기를 괄호로 함께 적는다(예: '15억 위안(150,000.00万元)', '2.66억 위안(RMB266.49 million)'). 원문 표기가 없는 금액은 쓰지 않는다.",
  "occurred_at과 occurred_precision은 이 사실이 언제의 것인지를 나타낸다. 두 종류를 반드시 구분한다.",
  "기간 집계(연간 매출, 연간 판매량, 기간 말 생산능력처럼 한 기간을 합산하거나 기간 말 기준으로 잰 값)는 occurred_at에 보고 기간 말일을 쓰고 occurred_precision을 year 또는 half로 둔다. 특정 하루에 일어난 일이 아니기 때문이다.",
  "시점 사건(투산·가동 개시, 착공, 신제품 출시, 고객 인증 획득, 계약 체결, 양산 개시처럼 특정 시점에 일어난 일)은 원문에 적힌 실제 발생일을 쓴다. 원문이 날짜까지 밝히면 occurred_precision을 day로, 월까지만 밝히면 그 달 1일과 month로, 상반기·하반기까지만 밝히면 그 반기 말일과 half로 둔다. 보고 기간 말일로 밀어 넣지 않는다.",
  "원문이 시점 사건의 시기를 전혀 밝히지 않으면 보고 기간 말일과 year를 쓰되, occurred_basis에 원문에 시점 표기가 없었다고 적는다.",
  "occurred_basis에는 그 날짜를 그렇게 정한 근거를 한 문장으로 쓴다(예: 원문에 2025년 3월 발표로 적힘 / 연간 집계라 기간 말일 사용).",
  // 공시는 공개 자료라 발췌를 400자까지 둘 수 있다. 언론 기사(300자)와 달리 저작권 제약이 없다.
  "original_excerpt에는 그 건의 근거가 된 보고서 원문을 400자 이내로 그대로 발췌한다. fact_ko에 쓴 수치는 하나도 빠짐없이 이 발췌 안에 원문 표기 그대로 있어야 한다. 발췌에 없는 수치가 생기면 그 수치를 fact_ko에서 빼거나, 그 수치가 있는 원문을 근거로 별개 건을 만든다. original_excerpt_ko에는 그 발췌문의 충실한 한국어 번역만 쓴다.",
  "original_excerpt는 입력에 있는 한 덩어리의 연속 원문을 글자 하나도 바꾸거나 줄이거나 말줄임표로 잇지 말고 그대로 복사한다. 표의 경우에는 같은 행의 값만 쓰며, 표 머리·단위가 입력에 없으면 그 표의 금액을 사실로 만들지 않는다. 이미지·차트는 이 입력에 포함되지 않으므로 그 안의 수치나 추세를 추정해 쓰지 않는다.",
  "source_url과 source_name은 빈 문자열로 둔다. 서버가 보고서 정보로 채운다.",
  "본문에 마크다운 링크나 인용 표기를 넣지 않는다.",
  LAYER_PROMPT_GUIDE,
].join(" ");

// 보고서 본문을 한 번에 넣으면 앞부분에 치우쳐 뒤쪽 사실을 빠뜨린다.
// 문단 경계에서 청크로 나눠 각각 뽑고, 같은 사실이 두 청크에 걸쳐 두 번 나오면 하나로 합친다.
const DIGEST_CHUNK_CHARS = 12000;

// 프롬프트를 고치면 그 전에 뽑힌 행은 옛 규칙의 결과다. 장부에 이 값을 남겨야
// "어느 보고서를 다시 읽어야 하는가"를 추측이 아니라 조회로 정한다.
export const DIGEST_PROMPT_VERSION = crypto.createHash("sha256").update(DIGEST_INSTRUCTIONS).digest("hex").slice(0, 12);

function splitSection(text, size = DIGEST_CHUNK_CHARS) {
  const chunks = [];
  let start = 0;
  while (start < text.length) {
    let end = Math.min(text.length, start + size);
    if (end < text.length) {
      const boundary = text.lastIndexOf("\n", end);
      if (boundary > start + size * 0.5) end = boundary + 1;
    }
    chunks.push(text.slice(start, end));
    start = end;
  }
  return chunks;
}

function eventDedupeKey(event) {
  return String(event.title_ko || "").replace(/[\s\p{P}\p{S}]/gu, "").toLowerCase();
}

// timeoutMs: 조각 하나의 LLM 호출 상한. 호출자(유지 훅)가 자기 예산에 맞춰 준다. 조각은 병렬로 돌므로
// 보고서 하나의 소요는 가장 느린 조각 하나와 같다.
export async function digestReport({ company, kind = "annual", knownUrl = null, maxEvents = 10, provider = "auto", timeoutMs, diagnostic = false }) {
  const report = await readReport(company, { kind, knownUrl });
  const entities = groupSearchEntities(company.id);
  const groupLine = entities.length ? `\n이 회사의 주요 계열사: ${entities.join(" / ")}` : "";
  const header = `회사: ${company.name_ko} [${company.name_zh}]${groupLine}\n보고서: ${report.title || REPORT_KINDS[kind]?.label_ko}${report.published_at ? ` (공시일 ${report.published_at})` : ""}`;
  const chunks = splitSection(report.section);
  // 조각 하나가 타임아웃으로 죽어도 나머지 조각의 사실은 살린다. 예전에는 Promise.all이라
  // 마지막 조각 하나의 실패가 앞 조각들의 결과까지 통째로 버렸고 그 보고서는 0건으로 남았다.
  // 실패한 조각 수를 돌려줘 호출자가 "이 결과는 부분이다"를 알고 옛 이벤트를 지우지 않게 한다.
  const settled = await Promise.allSettled(chunks.map((chunk, index) => createJsonResponse({
    name: "company_report_digest",
    schema: eventSchema(maxEvents),
    instructions: DIGEST_INSTRUCTIONS,
    input: `${header}\n(원문 ${index + 1}/${chunks.length} 부분)\n\n--- 보고서 원문 발췌 ---\n${chunk}`,
    provider,
    timeoutMs,
    skipTelemetry: diagnostic,
  })));
  const parts = [];
  const chunkErrors = [];
  // 조각별 산출 건수. null은 그 조각이 실패했다는 뜻이다. 앞 조각에만 쏠려 있으면
  // 조각이 너무 커서 뒤쪽 사실을 놓치고 있다는 신호다(DIGEST_CHUNK_CHARS를 줄인다).
  const perChunkReturned = settled.map((outcome, index) => {
    if (outcome.status !== "fulfilled") {
      chunkErrors.push({ chunk: index + 1, message: String(outcome.reason?.message || outcome.reason).slice(0, 160) });
      return null;
    }
    parts.push({ result: outcome.value, sourceChunk: chunks[index] });
    return (outcome.value.data.events || []).length;
  });
  // 조각이 전부 죽었으면 읽기 자체가 실패한 것이다. 호출자의 타임아웃 재시도 판단에 걸리도록 올려보낸다.
  if (!parts.length) throw settled[0].reason;
  const merged = new Map();
  for (const part of parts) {
    for (const event of part.result.data.events || []) {
      const key = eventDedupeKey(event);
      if (key && !merged.has(key)) merged.set(key, { event, sourceChunk: part.sourceChunk });
    }
  }
  const result = { data: { events: [...merged.values()] }, provider: parts[0]?.result.provider };

  const evidenceKind = kind === "annual" ? "annual_report" : "periodic_report";
  const reportName = `${company.name_ko} ${report.title || REPORT_KINDS[kind]?.label_ko || "정기보고서"}`;
  const rows = [];
  const dropped = { badDate: 0, empty: 0, ungroundedExcerpt: 0, badDateSamples: [] };
  for (const candidate of result.data.events || []) {
    const { event, sourceChunk } = candidate;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(event.occurred_at)) {
      dropped.badDate += 1;
      // 어떤 표기로 왔는지 남긴다. '2026-03'이나 '2026年3月'이 많으면 프롬프트가 아니라
      // 정규화가 문제다. 표본이 없으면 그 구분을 영영 못 한다.
      if (dropped.badDateSamples.length < 5) dropped.badDateSamples.push(String(event.occurred_at || "").slice(0, 40));
      continue;
    }
    const title = stripCitations(event.title_ko);
    const fact = stripCitations(event.fact_ko);
    if (!title || !fact) {
      dropped.empty += 1;
      continue;
    }
    if (!isGroundedReportExcerpt(event.original_excerpt, sourceChunk)) {
      dropped.ungroundedExcerpt += 1;
      continue;
    }
    rows.push({
      company_id: company.id,
      occurred_at: event.occurred_at,
      occurred_precision: event.occurred_precision || "day",
      occurred_basis: event.occurred_basis || null,
      title_ko: title,
      fact_ko: fact,
      trajectory_track: trackFor(normalizeLayerKey(event.layer_key), event.trajectory_track),
      layer_key: normalizeLayerKey(event.layer_key),
      region_scope: event.region_scope || null,
      // 출처는 모델이 아니라 서버가 정한다. 읽은 보고서 그 자체다.
      source_url: report.url,
      source_name: reportName,
      original_excerpt: stripCitations(event.original_excerpt),
      original_excerpt_ko: stripCitations(event.original_excerpt_ko),
      timeline_eligibility: "core",
      evidence_kind: evidenceKind,
      entity_names: matchGroupEntities(company.id, `${title} ${fact} ${event.original_excerpt}`),
    });
  }
  // 원문(text)과 분석에 실제로 넣은 구간(section)을 함께 돌려준다. 호출자가 장부에 보관하고
  // 벡터에 넣는다. 거래소에 공개된 자료라 남겨도 되고, 남겨야 나중에 다시 내려받지 않는다.
  // 품질 메타(parse_quality·visual_pages)와 원본 해시도 그대로 넘긴다. 예전에는 여기서 빠져
  // 호출자가 `report.parse_quality`를 읽어도 undefined였고, 이미지 도표가 있는 보고서까지
  // 전부 text_only·해시 없음으로 기록됐다.
  return { rows, dropped, returned: (result.data.events || []).length, provider: result.provider,
    // 진단값. 호출자가 report_digest 장부에 그대로 적는다(supabase/report-digest-diagnostics.sql).
    diagnostics: {
      section_chars: report.section?.length ?? null,
      digest_chunks: chunks.length,
      digest_chunks_failed: chunkErrors.length,
      digest_returned: (result.data.events || []).length,
      digest_dropped_bad_date: dropped.badDate,
      digest_dropped_empty: dropped.empty,
      digest_dropped_ungrounded_excerpt: dropped.ungroundedExcerpt,
      digest_prompt_version: DIGEST_PROMPT_VERSION,
    },
    per_chunk_returned: perChunkReturned,
    chunk_errors: chunkErrors,
    report: { url: report.url, title: report.title, published_at: report.published_at, pages: report.pages, text: report.text, section: report.section, parse_quality: report.parse_quality, visual_pages: report.visual_pages, source_sha256: report.source_sha256 } };
}


// 이미 저장된 연차보고서 이벤트의 시점을 다시 확인한다.
//
// 연차보고서에서 뽑은 사실은 기간 집계와 시점 사건이 섞여 있는데, 처음 적재할 때는 둘 다
// 보고 기간 말일로 들어갔다. 그래서 "2세대 블레이드 배터리 출시"처럼 그 해 3월에 있었던 일이
// 12월 31일에 일어난 것처럼 보인다. 기간 집계는 말일이 맞으므로 건드리지 않고,
// 시점 사건만 웹 검색으로 실제 시기를 찾아 고친다. 찾지 못하면 날짜를 바꾸지 않고
// 연 단위로만 믿는다고 표시한다. 모르는 것을 아는 척 옮기지 않기 위함이다.
const REDATE_SCHEMA = {
  type: "object", additionalProperties: false, required: ["items"],
  properties: {
    items: {
      type: "array", maxItems: 20,
      items: {
        type: "object", additionalProperties: false,
        required: ["id", "kind", "occurred_at", "occurred_precision", "basis_ko"],
        properties: {
          id: { type: "string" },
          kind: { type: "string", enum: ["period_aggregate", "point_event"] },
          occurred_at: { type: "string" },
          occurred_precision: { type: "string", enum: ["day", "month", "half", "year"] },
          basis_ko: { type: "string" }
        }
      }
    }
  }
};

export const REDATE_INSTRUCTIONS = [
  "중국 배터리 산업 애널리스트다. 이미 정리된 기업 이벤트 목록의 시점을 다시 판정한다.",
  "각 항목을 두 종류로 나눈다. period_aggregate는 한 기간을 합산하거나 기간 말 기준으로 잰 값이다(연간 매출, 연간 판매량, 기간 말 생산능력). point_event는 특정 시점에 일어난 일이다(투산·가동 개시, 착공, 신제품 출시, 고객 인증, 계약 체결, 양산 개시).",
  "period_aggregate는 occurred_at을 지금 값 그대로 두고 occurred_precision을 year로 한다. 기간 집계는 특정 하루의 일이 아니기 때문이다.",
  "point_event는 웹 검색으로 그 일이 실제로 언제 있었는지 찾는다. 회사명과 사건 내용을 함께 검색한다.",
  "날짜까지 확인되면 그 날짜와 day, 월까지만 확인되면 그 달 1일과 month, 분기·반기까지만 확인되면 그 기간 말일과 half를 쓴다.",
  "확인하지 못하면 occurred_at을 지금 값 그대로 두고 occurred_precision을 year로 한다. 추측으로 날짜를 옮기지 않는다.",
  "새로 정한 날짜는 보고 연도 1월 1일부터 이듬해 4월 말 사이여야 한다. 연차보고서는 보고 기간이 끝난 뒤 발행되므로 이듬해 초의 보고기간 후 사건이 실릴 수 있다.",
  "basis_ko에는 그렇게 판정한 근거를 한 문장으로 쓴다. 검색으로 확인했다면 무엇을 근거로 했는지 적는다.",
  "id는 입력에 주어진 값을 그대로 돌려준다. 바꾸거나 새로 만들지 않는다.",
].join(" ");

// 기사에서 뽑은 이벤트의 시점을 웹 검색으로 재확인한다.
//
// 기사 본문이 사건 시점을 적지 않으면 발행일이 그대로 들어간다. 특집·회고 기사는 몇 달 전 발표를
// 다시 소개하므로 시계열이 어긋난다(2026-09-04 기사가 소개한 비야디 2세대 블레이드는 2026-03-05 발표).
// 연차보고서용 재확인과 달리 보고 연도 제약이 없고, 대신 사건은 기사 발행일보다 뒤일 수 없다.
export const ARTICLE_REDATE_INSTRUCTIONS = [
  "중국 배터리 산업 애널리스트다. 언론 기사에서 뽑은 기업 이벤트의 실제 발생 시점을 다시 판정한다.",
  "각 항목은 기사 발행일이 시점으로 들어가 있다. 기사가 이미 발표된 사실을 다시 소개한 것이면 실제 발표·체결·가동·출시 시점은 그보다 앞선다.",
  "period_aggregate는 한 기간을 합산하거나 기간 말 기준으로 잰 값이다(반기 매출, 연간 출하량). 기간 말일과 half 또는 year를 쓴다.",
  "point_event는 웹 검색으로 회사 공식 발표·공시·1차 보도를 찾아 실제 시점을 확인한다. 회사명과 사건 내용을 함께 검색한다.",
  "날짜까지 확인되면 그 날짜와 day, 월까지만 확인되면 그 달 1일과 month, 반기까지만 확인되면 그 기간 말일과 half를 쓴다.",
  "확인하지 못하면 occurred_at을 지금 값 그대로 두고 occurred_precision을 month로 한다. 추측으로 날짜를 옮기지 않는다.",
  "새로 정한 날짜는 기사 발행일보다 뒤일 수 없다.",
  "basis_ko에는 판정 근거를 한 문장으로 쓴다. 검색으로 확인했다면 출처(매체·공식 발표·URL)를 적는다.",
  "id는 입력에 주어진 값을 그대로 돌려준다.",
].join(" ");

export async function redateArticleEvents({ company, events, provider = "openai" }) {
  const targets = events.filter((event) => event.title_ko && event.fact_ko).slice(0, 20);
  if (!targets.length) return { items: [], checked: 0 };
  const lines = targets.map((event) =>
    `id=${event.id} | 기사발행일=${event.article_published_at || event.occurred_at} | 현재시점=${event.occurred_at} | ${event.title_ko} | ${String(event.fact_ko).replace(/\s+/g, " ").slice(0, 220)}`
  );
  const result = await createJsonResponse({
    name: "article_event_redate",
    schema: REDATE_SCHEMA,
    instructions: ARTICLE_REDATE_INSTRUCTIONS,
    webSearch: true,
    provider,
    input: `회사: ${company.name_ko} [${company.name_zh}]\n\n판정할 이벤트:\n${lines.join("\n")}`,
  });
  const byId = new Map(targets.map((event) => [String(event.id), event]));
  const items = [];
  for (const item of result.data.items || []) {
    const original = byId.get(String(item.id));
    if (!original) continue;
    const proposed = /^\d{4}-\d{2}-\d{2}$/.test(item.occurred_at) ? item.occurred_at : original.occurred_at;
    const ceiling = String(original.article_published_at || original.occurred_at).slice(0, 10);
    // 기사보다 뒤이거나 터무니없이 오래된 날짜는 모델 오류로 보고 원래 값을 지킨다.
    const plausible = proposed <= ceiling && proposed >= "2015-01-01";
    const occurredAt = plausible ? proposed : original.occurred_at;
    items.push({
      id: original.id,
      occurred_at: occurredAt,
      occurred_precision: plausible ? item.occurred_precision : "month",
      occurred_basis: String(item.basis_ko || "").slice(0, 300) || "웹 검색 재확인 실행, 근거 미확보",
      changed: occurredAt !== original.occurred_at,
    });
  }
  return { items, checked: targets.length, provider: result.provider };
}

export async function redateReportEvents({ company, events, provider = "openai" }) {
  const targets = events.filter((event) => event.title_ko && event.fact_ko).slice(0, 20);
  if (!targets.length) return { items: [], checked: 0 };
  const lines = targets.map((event) =>
    `id=${event.id} | 현재시점=${event.occurred_at} | ${event.title_ko} | ${String(event.fact_ko).replace(/\s+/g, " ").slice(0, 220)}`
  );
  const years = [...new Set(targets.map((event) => String(event.occurred_at).slice(0, 4)))].join(", ");
  const result = await createJsonResponse({
    name: "event_redate",
    schema: REDATE_SCHEMA,
    instructions: REDATE_INSTRUCTIONS,
    webSearch: true,
    provider,
    input: `회사: ${company.name_ko} [${company.name_zh}]\n보고 연도: ${years}\n\n판정할 이벤트:\n${lines.join("\n")}`,
  });
  const byId = new Map(targets.map((event) => [String(event.id), event]));
  const items = [];
  for (const item of result.data.items || []) {
    const original = byId.get(String(item.id));
    if (!original) continue;
    const date = /^\d{4}-\d{2}-\d{2}$/.test(item.occurred_at) ? item.occurred_at : original.occurred_at;
    // 보고 연도를 벗어난 날짜는 모델이 잘못 짚은 것으로 보고 원래 값을 지킨다.
    // 연차보고서는 보고 기간이 끝난 뒤 발행되므로 "보고기간 후 사건"(예: 12월 결산 보고서에 적힌
    // 이듬해 3월 발표)이 실릴 수 있다. 보고 연도 1월 1일부터 이듬해 4월 말까지를 허용하고
    // 그 밖의 날짜는 모델이 잘못 짚은 것으로 보고 원래 값을 지킨다.
    const fiscalYear = Number(String(original.occurred_at).slice(0, 4));
    const sameYear = date >= `${fiscalYear}-01-01` && date <= `${fiscalYear + 1}-04-30`;
    const occurredAt = sameYear ? date : original.occurred_at;
    items.push({
      id: original.id,
      occurred_at: occurredAt,
      occurred_precision: item.kind === "period_aggregate" ? "year" : (sameYear ? item.occurred_precision : "year"),
      occurred_basis: String(item.basis_ko || "").slice(0, 300) || null,
      changed: occurredAt !== original.occurred_at,
    });
  }
  return { items, checked: targets.length, provider: result.provider };
}
