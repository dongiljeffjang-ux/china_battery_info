...
export async function digestReport({ company, kind = "annual", knownUrl = null, maxEvents = 10, provider = "auto" }) {
  ...
  return { rows, dropped, returned: (result.data.events || []).length, provider: result.provider, report: { url: report.url, title: report.title, published_at: report.published_at, pages: report.pages, text: report.text, section: report.section } };
}
