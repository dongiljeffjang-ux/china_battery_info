...
function signalsOf(article) {
  // LLM이 판단 근거를 남기지 않았거나 예전 기사라면 요약 문장으로 대신한다.
  // 근거가 비어 툴팁이 라벨만 보여주는 상황을 만들지 않기 위함이다.
  ...
  if (stored.length) {
    return stored
      .map((signal) => ({
        ...
        reason: String((signal && signal.reason_ko) || "").trim() || fallbackReason
      }))
  ...
  return (article.keywords_ko || [])
    .map((keyword) => ({ keyword: normalizeSankeyKeyword(keyword), direction, reason: fallbackReason }))
