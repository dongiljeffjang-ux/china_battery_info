import { NAME_TERMS } from "./signal-subjects.js";
  ...
  if (stored.length) {
    return stored
      ...
      .map((signal) => ({
        ...
        // 모델이 판정한 주체(lib/signal-subjects.js). 없으면 이름 매칭으로 가린다.
        subjectScope: signal.subject_scope || null,
        subjectIds: Array.isArray(signal.subject_company_ids) ? signal.subject_company_ids : [],
      }))
...
// ── 신호의 주체 회사 ────────────────────────────────────────────────────────
// 기사 하나가 여러 회사에 연결돼 있으면, 예전에는 그 기사의 모든 신호를 연결된 모든 회사에 붙였다.
// 2026-09-11 확인: "닝더스다이 주가 하락의 배경"(byd·calb·catl 연결)의 축소 신호는 리샹이 CATL 배터리를
// 빼고 자체·신왕다 전지로 바꿨다는 CATL 얘기인데, 비야디·중촹신항에도 같은 축소 선이 그려져 "기술·제품
// 축소 3건"으로 셌다(실제로는 1건). 중촹신항은 같은 기사의 확대 신호(샤오미와 룽자 배터리 발표)의 주체였다.
// 그래서 신호 근거 문장에 이름이 나오는 연결 회사에만 그 신호를 붙인다. 아무도 이름이 없으면 예전처럼 전체.
export function signalSubjects(text, links = []) {
  const lower = String(text || "").toLowerCase();
  if (!lower) return links;
  const named = links.filter((link) => (NAME_TERMS.get(link.company_id) || []).some((term) => lower.includes(term)));
  return named.length ? named : links;
}

function collect(deduped, article, signals, grade) {
  for (const signal of signals) {
    // 검증 기사는 신호 근거 문장(과 키워드)으로, 미검증 헤드라인은 제목으로 주체를 가린다.
    const subjectText = grade === "headline" ? `${article.title_ko || ""} ${article.title_original || ""}` : `${signal.keyword || ""} ${signal.reason || ""}`;
    const links = article.article_company || [];
    // 1순위: 모델이 고른 주체. 산업 전반·추적 외 회사 신호는 회사 선을 그리지 않는다.
    // 2순위(주체 판정이 없는 예전 기사·미검증 헤드라인, 또는 이름을 연결 회사로 못 되돌린 경우): 이름 매칭.
    let subjects;
    if (signal.subjectScope === "industry" || signal.subjectScope === "other_company") subjects = [];
    else if (signal.subjectScope === "company" && signal.subjectIds.length) subjects = links.filter((link) => signal.subjectIds.includes(link.company_id));
    else subjects = signalSubjects(subjectText, links);
