...
const nameTerms = new Map(COMPANIES.map((company) => {
  const base = String(company.name_ko || "").replace(/\s*\(.*?\)\s*/g, "").trim();
  const paren = (String(company.name_ko || "").match(/\(([^)]+)\)/) || [])[1];
  const terms = [base, paren, company.name_zh, company.name_en, ...(company.aliases || [])]
...
export function signalSubjects(text, links = []) {
  ...
  const named = links.filter((link) => (nameTerms.get(link.company_id) || []).some((term) => lower.includes(term)));
...
function collect(deduped, article, signals, grade) {
  for (const signal of signals) {
    ...
    for (const link of signalSubjects(subjectText, article.article_company || [])) {
