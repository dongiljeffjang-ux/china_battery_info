...
export const THEMES = [
  ...
  },
  {
    ...
    expansion: ["가격 인상", "가격 상승", "판가 인상", "단가 인상", "가격 반등", "탄산리튬 가격 상승", "리튬 가격 상승", "价格上涨", "涨价", "提价", "价格回升", "碳酸锂涨价", "锂价上涨", "price increase", "price hike", "prices rose"],
...
function includesTerm(text, compactText, term) {
  ...
  return text.includes(lowered) || compactText.includes(lowered.replace(/\s+/g, ""));
}
...
export function sankeyFlowsFromArticles(verified = [], headlines = []) {
  ...
  for (const article of headlines) collect(deduped, article, headlineSignalsOf(article), "headline");
