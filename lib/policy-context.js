...
// 정책 전체 연혁은 화면용이고, 보고서 모델에는 기준일 전후 1년의 대표 항목만 보낸다.
// 과거 정책의 시행·유예 일정은 historicalPolicies()가 별도 시점 행으로 만들어 두므로,
// 이 창 안의 일정도 함께 후보가 된다. 중요도를 추정하지 않고 시간축에서 고르게 표본화한다.
const POLICY_TOPICS = [
  { terms: ['수출', '해외', '통제', '환급', '통상'], tags: [] },
  { terms: ['안전', '품질', '인증', '표준', 'GB', 'CCC'], tags: ['cell'] },
  { terms: ['ESS', '저장', '에너지저장'], tags: ['cell'] },
  { terms: ['전기차', '신에너지차', '동력전지', 'PHEV'], tags: ['cell'] },
  { terms: ['양극', 'LFP', '리튬'], tags: ['cathode'] },
  { terms: ['음극', '흑연', '리튬'], tags: ['anode'] },
  { terms: ['제조', '생산', '산능', '가동'], tags: [] },
];

export function selectReportPolicies(policies = [], events = [], { limit = 8, companyTags = [] } = {}) {
  const dates = events.map(event => String(event?.date || event?.occurred_at || '')).filter(date => /^\d{4}-\d{2}-\d{2}$/.test(date)).sort();
  const latest = dates.at(-1);
  const rows = [...policies].filter(row => /^\d{4}-\d{2}-\d{2}$/.test(String(row?.occurred_at || '')))
    .sort((a, b) => String(a.occurred_at).localeCompare(String(b.occurred_at)));
  if (!latest || rows.length <= limit) return rows.slice(0, limit);
  const pivot = new Date(`${latest}T00:00:00Z`);
  const from = new Date(pivot); from.setUTCDate(from.getUTCDate() - 365);
  const until = new Date(pivot); until.setUTCDate(until.getUTCDate() + 365);
  const inWindow = rows.filter(row => {
    const date = new Date(`${row.occurred_at}T00:00:00Z`);
    return date >= from && date <= until;
  });
  const candidates = inWindow.length ? inWindow : rows.filter(row => row.occurred_at <= latest);
  const eventText = events.map(event => `${event?.title || ''} ${event?.fact || ''}`).join(' ');
  const tags = new Set(companyTags);
  const score = row => {
    const text = `${row?.title_ko || ''} ${row?.fact_ko || ''}`;
    return POLICY_TOPICS.reduce((total, topic) => {
      const matchesEvent = topic.terms.some(term => eventText.includes(term));
      const matchesPolicy = topic.terms.some(term => text.includes(term));
      const matchesCompany = topic.tags.some(tag => tags.has(tag));
      return total + (matchesPolicy && (matchesEvent || matchesCompany) ? 3 : 0);
    }, 0);
  };
  // 같은 정책의 발표·시행 일정은 한 행으로 대표한다. 행 안에 announced/effective가 모두 남는다.
  const deduped = [...candidates].sort((a, b) => score(b) - score(a) || String(b.occurred_at).localeCompare(String(a.occurred_at)))
    .filter((row, index, all) => !all.slice(0, index).some(other => String(other.id || '').replace(/-schedule-.*/, '') === String(row.id || '').replace(/-schedule-.*/, '')));
  return deduped.slice(0, limit).sort((a, b) => String(a.occurred_at).localeCompare(String(b.occurred_at)));
}
