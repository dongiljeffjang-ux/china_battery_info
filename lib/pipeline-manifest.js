...
import { COMPANIES, SELECTION_BASIS, buildSearchGroups, plannedSearchRequests, searchProviderPrompt, searchInstructions } from "./china-sources.js";
...
import { ARTICLE_ANALYSIS_PROMPT_BODY, ARTICLE_FACT_CHECK_PROMPT, ARTICLE_DATE_GUIDE } from "../api/process-article.js";
...
function tagged(tag) {
  return COMPANIES.filter((company) => company.type_tags.includes(tag)).length;
}

export function pipelineManifest() {
  const openaiGroups = buildSearchGroups("openai", false);
  const deepseekGroups = buildSearchGroups("deepseek", false);
  ...
  return {
    ...
    sources: [
      ...
      {
        id: "web_search_deepseek",
        label: "DeepSeek 웹 검색",
        what: "같은 기간을 중국 현지 매체·지방정부·기업 발표 중심으로 독립 검색한다.",
        endpoint: "DeepSeek Responses API · web_search 도구",
        model: model("DEEPSEEK_MODEL", "deepseek-v4-flash"),
        api_key: env("DEEPSEEK_API_KEY"),
        groups: deepseekGroups.length,
        companies_per_group: `최대 ${Math.max(...deepseekGroups.map((g) => g.ids.length))}`,
        group_labels: deepseekGroups.map((g) => `${g.label} (${g.ids.length}곳)`),
        stores: "article (verification_status=pending, source_tier=web_search_deepseek)",
        note: "묶음을 작게 두는 이유: 회사가 많으면 목표 건수를 채우려 검색을 반복하다 35초 제한을 넘긴다.",
      },
      {
        id: "catl_newsroom",
        label: "CATL 뉴스룸",
      ...
      },
      {
        ...
        companies: COMPANIES.filter((c) => c.cninfo).length,
    ...
    stages: [
      {
        ...
        prompts: [
          ...
          { name: "제공자별 지시 · OpenAI", provider: "openai", text: searchProviderPrompt("openai") },
          { name: "제공자별 지시 · DeepSeek", provider: "deepseek", text: searchProviderPrompt("deepseek") },
        ],
      },
      {
        id: "select",
      ...
      },
      {
        ...
        what: "원문을 받아 OpenAI가 한국어 사실과 이벤트를 뽑고, DeepSeek이 같은 원문으로 대조한다. 통과해야 화면에 올라간다.",
        prompts: [
          ...
          { name: "교차검증", provider: "deepseek", text: ARTICLE_FACT_CHECK_PROMPT },
      ...
      },
      {
        ...
        prompts: [{ name: "Daily 편집", provider: "openai", text: DAILY_REPORT_PROMPT }],
      },
      {
        id: "curate",
        label: "5. 유지 (야간)",
