...
// 시장 레이어의 정기보고서는 생산능력·출하량·매출·영업이익에 해당하는 핵심
// 정량 신호만 남긴다. 기술 레이어의 정기보고서는 개발·특허·공정·인증처럼
// 정량 키워드가 없는 사실도 사업 시계열의 핵심이므로 보존한다.
// 순이익·연구개발·특허 수치는 시장 레이어에 있어도 핵심으로 보인다(사용자 결정 2026-09-10).
const REPORT_TIMELINE_QUANT = /생산(?:능력|용량|캐파)|캐파|출하(?:량)?|판매량|영업이익|순이익|매출(?!원가|총이익|채권)|产能|出货(?:量)?|销量|营业利润|净利润|营业(?:总)?收入|营收|연구개발|R&D|研发|특허|专利/iu;
const REPORT_CUSTOMER_BREAKDOWN = /상위\s*\d+대\s*고객|\d+위\s*고객|고객.*(?:판매금액|매출)|前\s*五大客户|第\s*[一二三四五1-5]\s*大客户/iu;

export function isTimelineBusinessEvent(event) {
  const text = `${event.title_ko || ""} ${event.fact_ko || ""}`;
  const technicalReport = REPORT_EVIDENCE.has(event?.evidence_kind)
    && (event?.trajectory_track === "technology"
      || event?.trajectory_track === "both"
      || String(event?.layer_key || "").startsWith("technology-"));
  if (REPORT_EVIDENCE.has(event?.evidence_kind)) {
    if (technicalReport) return true;
    if (REPORT_CUSTOMER_BREAKDOWN.test(text)) return false;
    if (DISCLOSURE_NOISE.test(text)) return false;
    return REPORT_TIMELINE_QUANT.test(text);
  }
  if (DISCLOSURE_NOISE.test(text)) return false;
