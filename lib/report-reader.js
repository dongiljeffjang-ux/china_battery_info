...
export async function extractPdfText(url) {
  ...
  return { text, pages: doc.numPages, visual_pages: visualPages, parse_quality: visualPages.length ? "visual_review_required" : "text_only" };
}
...
export async function readReport(company, { kind = "annual", knownUrl = null } = {}) {
  ...
  const { text, pages, visual_pages, parse_quality } = await extractPdfText(found.url);
  ...
  return { ...found, pages, section, text, totalChars: text.length, visual_pages, parse_quality };
}
