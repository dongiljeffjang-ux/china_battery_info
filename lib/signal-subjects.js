...
// 판정 규칙 버전. 규칙을 바꾸면 올리고, 영향을 받는 예전 판정만 재판정한다(REJUDGE_SCOPES).
// v2(2026-09-11): 완성차가 공급처·협력사로 정한 추적 회사를 확대 신호의 주체로 본다 → 예전에 추적 외
// 회사(other_company)로 판정된 신호만 다시 본다.
export const SUBJECT_RULE_VERSION = 2;
const REJUDGE_SCOPES = new Set(["other_company"]);
export function needsSubjectJudgement(signal) {
  if (!signal || signal.direction === "neutral") return false;
  if (!signal.subject_scope) return true;
  return (Number(signal.subject_rule_version) || 1) < SUBJECT_RULE_VERSION && REJUDGE_SCOPES.has(signal.subject_scope);
}
...
export const SIGNAL_SUBJECT_RULE =
  ...
  "완성차·고객사가 공급처를 새로 정하거나 늘리거나 협력사·합작 상대로 발표한 신호는, 결정한 쪽이 목록 밖 회사여도 공급사·협력사로 선정·추가된 [연결 회사]를 확대 신호의 주체로 적는다(예: '샤오미가 중촹신항·신왕다와 룽자 배터리 발표' → 중촹신항·신왕다). 공급에서 빠진 [연결 회사]는 축소 신호의 주체다. " +
...
export function attachSubjects(signals = [], linkedIds = []) {
  return (signals || []).map((signal) => {
    ...
    if (scope !== "company") return { ...signal, subject_scope: scope, subjects_ko: names, subject_company_ids: [], subject_rule_version: SUBJECT_RULE_VERSION };
    const ids = resolveSubjectIds(names, linkedIds);
    return { ...signal, subject_scope: ids.length ? "company" : "unresolved", subjects_ko: names, subject_company_ids: ids, subject_rule_version: SUBJECT_RULE_VERSION };
  });
}
...
function backfillInput(batch) {
  return batch.map((article, index) => [
    ...
    "신호:",
    // 판정이 필요한 신호만 번호와 함께 보인다. 번호는 저장된 순서 그대로라 되돌릴 때 어긋나지 않는다.
    ...(article.headline_signals || []).map((signal, i) => needsSubjectJudgement(signal) ? `  ${i}. ${signal?.direction || ""} · ${signal?.keyword_ko || ""} · ${signal?.reason_ko || ""}` : "").filter(Boolean),
  ].join("\n")).join("\n\n");
}
...
export function applyBackfill(article, judged) {
  ...
  const merged = (article.headline_signals || []).map((signal, index) => {
    ...
    if (!signal || !verdict || !needsSubjectJudgement(signal)) return { signal, judged: false };
    return { signal: { ...signal, subject_scope: verdict.subject_scope, subjects_ko: verdict.subjects_ko }, judged: true };
  });
  // 이번에 판정한 신호만 서버 판정을 다시 붙인다. 판정이 필요 없던 신호는 저장된 그대로 둔다.
  return merged.map(({ signal, judged }) => judged ? attachSubjects([signal], article.linked_ids)[0] : signal);
}
...
export async function backfillSignalSubjects({ limit = 40, batchSize = BACKFILL_BATCH, deadline = Infinity } = {}) {
  ...
  const pending = (rows || [])
    ...
    .filter((row) => Array.isArray(row.headline_signals) && row.headline_signals.some(needsSubjectJudgement));
  ...
  for (let start = 0; start < todo.length; start += batchSize) {
    ...
    try {
      ...
      for (const [index, article] of batch.entries()) {
        ...
        const complete = !signals.some(needsSubjectJudgement);
