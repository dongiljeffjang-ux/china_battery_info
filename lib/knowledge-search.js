...
export async function answerFromKnowledge({ question, companyId = null, includeUnverified = false }) {
  ...
  // 이 화면은 생성형 RAG가 아니라 검색 전용이다. 질문 임베딩으로 후보를 찾고, 저장된 근거를
  // 관련도 순으로 그대로 돌려준다. 여기서 Luna·Terra 등 텍스트 생성 모델을 호출하지 않는다.
  ...
  return { mode: "retrieval_only", sufficient: true, sources, matched: chunks.length, unverified_matched: unverified,
    retrieval,
