...
import { matchPeriods, parseMetricQuestion, lookupMetrics } from "./metric-lookup.js";
...
export function demoteUnrelatedCompanies(rows, companyTerms, groups = null) {
  ...
  // 회사별 묶음이 있으면 "몇 회사를 언급했나"로 층을 나눈다. 두 회사를 물었을 때 둘 다 나오는 근거가
  ...
  const scored = rows.map((row, index) => {
    ...
    const hits = tiers.filter((group) => group.some((term) => containsTerm(text, term))).length;
    return { row, index, hits };
  });
...
export async function searchKnowledge({ question, intentQuestion = question, companyId = null, limit = MATCH_COUNT, includeUnverified = false }) {
  ...
  const requiredGroups = companyId ? [] : questionCompanyGroups(intentQuestion);
...
export function sanitizeRewrittenQueries(original, queries) {
  ...
  return [...new Set((queries || []).map((query) => String(query || "").replace(/\s+/g, " ").trim())
    .filter((query) => query.length >= 2 && query.length <= 180 && query !== source))].slice(0, 3);
}
