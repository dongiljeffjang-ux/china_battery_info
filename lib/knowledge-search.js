...
// 로컬 채점 실험용 덮어쓰기. 운영에는 이 환경변수가 없어 아래 기본값이 그대로 쓰인다.
// scripts/eval-benchmark-local.mjs를 여러 설정으로 돌려 "어느 값이 54문항에서 나은가"를 숫자로 고르기 위한 것이다.
const tuned = (name, fallback) => { const value = Number(process.env[name]); return Number.isFinite(value) && value > 0 ? value : fallback; };
// 2026-09-10 54문항 로컬 채점: K=10 대비 K=30이 MRR 0.661→0.676, Hit@10·Recall 동일(하락 없음). 위의 "10이어야
// 한다"는 2026-09-09 판정 42건에서 나온 추정이었고, 문항이 늘자 겹친 근거를 더 부드럽게 올리는 30이 나았다.
// 60은 재지 않았다. 다음 실측에서 갈아끼운다.
const RRF_K = tuned("RAG_RRF_K", 30);
...
const VECTOR_WEIGHT = tuned("RAG_VECTOR_WEIGHT", 0.7);
const LEXICAL_WEIGHT = tuned("RAG_LEXICAL_WEIGHT", 0.3);
// 단어 검색 필수 그룹에 넣는 회사 표기 상한. buildLexicalQuery 참고.
const REQUIRED_GROUP_TERM_LIMIT = 200;
// 회사를 짚지 않은 질문에서 단어 검색에서만 걸린 상위 근거를 최종 10건에 반드시 넣는 칸 수.
// K=10·0.7/0.3에서는 단어 검색 1위(0.3/11=0.027)가 의미 검색 10위(0.7/20=0.035)에도 못 미쳐, 의미 검색이
// 10건을 채우면 단어 검색 단독 근거는 어떤 순위여도 최종 10건에 못 들어간다(2026-09-10 실측: "전구체 합작 투자
// 사례"의 정답이 단어 검색 2위·8위인데 융합 12위·18위). 가중치를 0.5/0.5로 올리면 단어 검색 잡음이 전면에
// 퍼져 54문항에서 Hit@10이 0.90→0.76으로 떨어졌다. 그래서 전체를 다시 섞지 않고 칸 수만 보장한다.
// 0이면 예전과 같다. 회사를 짚은 질문은 소속 층 정렬이 이미 단어 검색 근거를 끌어올리므로 적용하지 않는다.
const LEXICAL_RESERVE = tuned("RAG_LEXICAL_RESERVE", 0);
// '보조 데이터 포함'을 켰을 때 검증 근거 뒤에 덧붙이는 미검증 헤드라인 칸 수.
// 헤드라인은 한 줄짜리라 같은 풀에서 경쟁시키면 짧은 문장이 유사도를 쉽게 얻어 검증 근거를 밀어낸다
// (2026-09-10 실측: "적자를 보고 있는 중국 기업?"에서 정답이던 Farasis 순손실 청크가 10위에서 완전히
// 빠지고 헤드라인 4건이 그 자리를 차지했다). 포함은 더하는 것이지 바꾸는 것이 아니므로, 검증 근거는
// 토글을 끈 것과 똑같이 뽑고 헤드라인은 그 뒤에 별도 칸으로 붙인다.
const HEADLINE_QUOTA = 3;
...
export function buildLexicalQuery(question, { requiredTerms = [], requiredGroups = [] } = {}) {
  ...
  // 필수 그룹의 상한은 넉넉히 둔다. 밸류체인 그룹("셀사", "양극재 회사")은 회사 10여 곳의 표기를 전부 담아
  // 98~156개가 되는데, 예전 상한 40에 잘려 앞 4~5개 회사만 필수 조건에 남았다(2026-09-10 실측: 양극재 그룹
  // 156개 중 40개 안에 CATL·Farasis·CNGR·Gotion 표기가 없어 "모로코 양극재 공장" 정답(CNGR)이 단어 검색에서
  // 통째로 빠졌다). 표기는 회사 마스터에서만 오므로 사용자 입력 길이 방어와는 무관하다.
  const groups = (requiredGroups.length ? requiredGroups : [requiredTerms])
    .map((group) => [...new Set((group || []).map(asPhraseTerm).filter(Boolean))].slice(0, REQUIRED_GROUP_TERM_LIMIT))
...
async function vectorSearch({ question, companyId, limit, includeUnverified, embedding: given = null }) {
  // given은 값일 수도 약속일 수도 있다. 약속이면 두 벡터 패스가 같은 임베딩 호출 하나를 기다린다.
  const embedding = given ? await given : await embedQuestion(question);
...
export async function searchKnowledge({ question, intentQuestion = question, companyId = null, limit = MATCH_COUNT, includeUnverified = false }) {
  ...
  // 검증 근거 검색은 토글과 무관하게 언제나 같은 조건으로 돈다. 보조 데이터는 아래에서 따로 가져와 덧붙인다.
  const args = { question: searchQuestion, companyId, limit: RETRIEVER_LIMIT, includeUnverified: false, requiredTerms, requiredGroups };
  ...
  // 임베딩은 질문당 한 번만 만들고 두 벡터 패스가 그 약속을 함께 기다린다. 단어 검색은 임베딩을 기다리지 않는다.
  const embedding = embedQuestion(searchQuestion);
  embedding.catch(() => {}); // 아래 allSettled가 결과를 받으므로 여기서는 미처리 거부만 막는다.
  // 보조 데이터 패스는 헤드라인만 건지면 되므로 넓게 뽑아 골라낸다. 토글이 꺼져 있으면 아예 부르지 않는다.
  const headlineArgs = { ...args, includeUnverified: true, limit: LEXICAL_POOL };
  const [vectorResult, lexicalResult, metricResult, headlineVectorResult, headlineLexicalResult] = await Promise.allSettled([
    vectorSearch({ ...args, embedding }),
    lexicalSearch(lexicalArgs),
    lookupMetrics(parsed, { supabaseRest, limit: METRIC_ROW_LIMIT }),
    includeUnverified ? vectorSearch({ ...headlineArgs, embedding }) : Promise.resolve([]),
    includeUnverified ? lexicalSearch(headlineArgs) : Promise.resolve([]),
  ]);
  ...
  const searchSlots = Math.max(0, limit - metricChunks.length);
  let picked = scopedOrdered.slice(0, searchSlots);
  // 단어 검색 단독 근거의 보장 칸. 융합 상위에 이미 단어 검색 근거가 있으면 그만큼 덜 채운다.
  if (LEXICAL_RESERVE > 0 && !companyTiers.length && searchSlots > LEXICAL_RESERVE) {
    const isLexicalOnly = (row) => (row.retrieved_by || []).length === 1 && row.retrieved_by[0] === "lexical";
    const already = picked.filter(isLexicalOnly).length;
    const need = LEXICAL_RESERVE - already;
    if (need > 0) {
      const pickedIds = new Set(picked.map((row) => row.id));
      const rescue = scopedOrdered.filter((row) => isLexicalOnly(row) && !pickedIds.has(row.id))
        .sort((a, b) => (a.ranks?.lexical ?? 99) - (b.ranks?.lexical ?? 99)).slice(0, need);
      if (rescue.length) picked = [...picked.slice(0, searchSlots - rescue.length), ...rescue];
    }
  }
  // 보조 데이터는 검증 근거를 밀어내지 않고 뒤에 붙는다. 두 패스 모두에서 헤드라인만 골라 따로 융합한다.
  const headlineRows = includeUnverified ? (() => {
    const onlyHeadlines = (rows) => (rows || []).filter((row) => row.source_type === "headline");
    const pickedIds = new Set([...metricChunks, ...picked].map((row) => row.id));
    return fuseByRrf([
      { rows: onlyHeadlines(headlineVectorResult.status === "fulfilled" ? headlineVectorResult.value : []), weight: VECTOR_WEIGHT, label: "vector" },
      { rows: onlyHeadlines(headlineLexicalResult.status === "fulfilled" ? headlineLexicalResult.value : []), weight: LEXICAL_WEIGHT, label: "lexical" },
    ]).filter((row) => !pickedIds.has(row.id)).slice(0, HEADLINE_QUOTA);
  })() : [];
  const top = [...metricChunks, ...picked, ...headlineRows];
  ...
  top.retrieval = {
    ...
    headline_rows: headlineRows.length,
...
const ANSWER_SCHEMA = {
  ...
  properties: {
    ...
    // 검증 근거 MATCH_COUNT칸 + 보조 데이터 HEADLINE_QUOTA칸. 헤드라인을 덧붙이면 번호가 10을 넘는다.
    used_sources: { type: "array", maxItems: MATCH_COUNT + HEADLINE_QUOTA, items: { type: "integer", minimum: 1, maximum: MATCH_COUNT + HEADLINE_QUOTA } },
  },
};
