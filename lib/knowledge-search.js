...
export async function searchKnowledge({ question, companyId = null, limit = MATCH_COUNT }) {
  const embedding = await embedQuestion(question);
  const rows = await supabaseRest("rpc/match_knowledge_chunks", {
    method: "POST",
    body: { query_embedding: embedding, match_count: limit, filter_company_id: companyId || null },
  });
...
export async function answerFromKnowledge({ question, companyId = null, provider = "auto" }) {
  const chunks = await traced("retrieve_knowledge", () => searchKnowledge({ question, companyId }), {
    runType: "retriever",
    inputs: { question, company_id: companyId },
    metadata: { company_id: companyId || "all" },
  });
  if (!chunks.length) {
    return {
      sufficient: false,
  ...
  const evidence = chunks.map((chunk, index) => [
    `[${index + 1}] 회사=${chunk.company_id || "미상"} 시점=${chunk.published_at || "미상"} 출처=${chunk.source_name || "미상"}`,
    chunk.content_ko,
  ...
  return { ...data, sources, matched: chunks.length };
}
