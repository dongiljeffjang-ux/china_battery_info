...
export function buildTimelineInput({ companyName, events, metrics = [], policies = [] }) {
  ...
  const selectedEvents = selectTimelineEvidence(events);
  const policySection = policies.length ? [
    "",
    "같은 기간의 중국 배터리·NEV·ESS 주요 정책(전역 정책축, 과거 → 최근):",
    "",
  ...
  return [
    ...
    quant || "(없음)",
    "",
    `화면 이벤트 ${events.length}건 중 기간·레이어별 대표 근거 ${selectedEvents.length}건(레이어별 표, 과거 → 최근):`,
    "",
