...
function layerTable(events) {
  ...
  const rows = periods.map(period => {
    const cells = GROUPS.map(group => events
      ...
      .map(event => cellText(event.title) + " — " + cellText(event.fact) + " (" + cellText(event.date) + (event.entity ? " · 발생 법인: " + cellText(event.entity) : "") + " · 출처: " + cellText(event.sourceName) + (event.sourceUrl ? ", " + event.sourceUrl : ", 링크 미제공") + ")")
...
export function buildTimelineInput({ companyName, companyTags = [], events, metrics = [], alternatives = [], policies = [] }) {
...
export async function buildTimelineReport({ companyName, companyTags = [], events, metrics = [], alternatives = [], policies = [] }) {
  const { data, model } = await createJsonResponse({
    ...
    input: buildTimelineInput({ companyName, companyTags, events, metrics, alternatives, policies }),
  });
