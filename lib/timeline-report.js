...
const REPORT_MODE_GUIDE = {
  direction: [
    ...
    "분석 단위는 '비교 가능한 과거 상태 대 현재 상태'다. [기준 시점 대비 후보]와 정량 시계열을 함께 읽고, 제품/화학계·지역·고객·투자·생산능력·기술 가운데 같은 대상·기간·집계 범위로 비교할 수 있는 축만 고른다.",
    "각 변화는 유지·확장·중심 이동 중 근거에 맞게 설명한다. 신규 사업 진입만 확인되면 확장으로 쓴다. 매출 비중·출하량·생산능력·투자 규모의 상대적 변화나 명시적인 자원 재배분이 확인될 때만 중심 이동 또는 전환으로 쓴다. 투자 계획의 재배분과 실제 매출 구조의 변화도 구별한다.",
    "변화와 함께 기존 주력이 유지되는 근거도 반영한다. 기업 전체가 전환했다는 결론을 미리 정하지 않는다. 이 보고서에서 사실은 유지·확장·이동을 가르는 증거로 쓴다.",
    ...
    "## 과거 대 현재",
    "| 변화 축 | 과거 (시점) | 현재 (시점) | 해석 |  ← 비교 가능한 축만 최대 3행. 해석은 유지·확장·중심 이동을 구분한다.",
    "## 방향을 만든 결정",
  ...
  pattern: [
    ...
    "분석 단위는 '독립된 사례에서 반복되는 구체적인 선택'이다. 제품·공장·지역·프로젝트를 사례 단위로 묶고, 서로 다른 사례 두 개 이상에서 되풀이되는 투자 순서·고객 확보 방식·합작과 자체 투자의 선택·생산기반 활용 방식 같은 행동 구조를 찾는다. 이 예시는 입력에 확인된 경우에만 사용한다.",
    "같은 프로젝트의 착공·인증·출하는 하나의 사례다. 같은 사건의 여러 기사도 하나로 센다. 한 사업의 단계 진척(톤급 → 10톤급, 건설 → 인증), 단일 사건, 한 시점의 현황, 과거 대 현재 대비는 패턴이 아니다.",
    "'개발 → 생산 → 출하'처럼 제조업의 일반적인 순서만으로 패턴을 만들지 않는다. 사례마다 실제로 확인된 단계만 비교하고, 한 사례의 빈 단계를 다른 사례로 채우지 않는다. 공통 행동과 어긋나는 사례가 있으면 적용 범위를 좁힌다. 경쟁사 근거 없이 기업 고유의 방식이라고 표현하지 않는다.",
    "[사업선별 사건 순서]는 사례 후보를 찾기 위한 기계적 묶음일 뿐 패턴의 증거가 아니다. 사건 표에서 독립 사례를 다시 확인하고, 같은 구체적 선택이 두 사례 이상에서 보일 때만 쓴다. 성립하는 패턴이 없으면 진척 요약을 대신 쓰지 않는다.",
    ...
    "## 패턴 1: (결론형 제목)",
    "**반복 행동:** (어떤 상황에서 어떤 구체적인 선택을 반복하는지 한 문장)",
    "| 독립 사례 | 확인된 선택·진행 (시점·사실) | 공통 구조 |  ← 서로 다른 사례를 최소 2행 비교한다.",
    "**이 구조가 말하는 것:** 두세 문장. 회사의 우선순위·역량·한계와 사업 전개 방식에 주는 의미를 설명한다. 입력에 예외가 있으면 적용 범위를 함께 좁힌다.",
  ...
  inflection_point: [
    ...
    "분석 단위는 '현재 근거·분기 조건·전략적 결과'다. [진행 상태 후보]에서 최신 후속 사실까지 읽어 현재 위치를 먼저 확정하고, 조건에 따라 회사의 역할·자원 배분·고객 기반·공급 방식이 질적으로 달라지는 지점만 분기로 삼는다.",
    ...
    "경로는 예언이 아니다. 이미 관측된 움직임에서 바로 이어질 수 있는 두 갈래를 조건부로 쓴다. 반복 주문·고객 채택·증설 결정은 앞으로 판별할 조건으로 쓸 수 있지만 이미 발생한 사실처럼 쓰지 않는다. 신호가 아직 없다는 이유만으로 반대 경로가 확정됐다고 쓰지 않는다. 성공 확률·미래 매출·임의의 수치 기준과 기한을 만들지 않는다.",
...
const MODE_READING = {
  direction: "읽는 순서: [기준 시점 대비 후보]와 정량 시계열의 비교 가능성을 먼저 확인하고, 사건 표에서 유지·확장·중심 이동 중 무엇인지 판단하라.",
  pattern: "읽는 순서: 사건 표에서 제품·공장·지역·프로젝트별 독립 사례를 먼저 묶어라. [사업선별 사건 순서]는 사례 후보를 찾는 데만 쓰고, 서로 다른 사례에서 같은 구체적 선택이 반복되는지 비교하라. 한 사례의 진척과 제조업의 일반적인 단계는 패턴으로 쓰지 마라.",
  inflection_point: "읽는 순서: [진행 상태 후보]의 사건을 사건 표의 후속 사실과 대조해 현재 위치를 확정하라. 그다음 조건에 따라 기업의 역할·자원 배분·고객 기반이 달라지는 두 경로를 써라.",
};
function modeAid(events, mode) {
  if (mode === "pattern") {
    const lines = businessLineTable(events);
    return ["[사업선별 사건 순서] 제목·사실의 제품·지역 어휘로 묶은 사례 후보. 기계적 묶음이며 패턴의 증거가 아니다. 같은 프로젝트의 진행은 한 사례로 세고, 사건 표에서 서로 독립된 사례인지 다시 확인한다:", "", lines || "(둘 이상의 사업선 후보 없음 — 사건 표에서 독립 사례를 직접 찾는다)"];
  }
  if (mode === "inflection_point") {
    const table = stageMapTable(events);
    return ["[진행 상태 후보] 다음 단계가 남은 것으로 보이는 사건. 제목·사실의 어휘로 고른 후보이므로, 이후 완료·변경 사건이 있는지 전체 사건 표에서 확인한 뒤 현재 상태를 판단한다:", "", table || "(진행 상태 후보 없음 — 사건 표에서 현재도 전략적 경로가 갈리는 사업을 직접 찾는다)"];
  }
  const table = baselineTable(events);
  return ["[기준 시점 대비 후보] 축별로 가장 오래된 연도와 최근 연도의 대표 사건. 같은 대상·기간·집계 범위인지 확인한 뒤 비교에 사용한다:", "", table || "(축별로 두 연도 이상 비교할 사건 없음 — 정량 시계열에서 비교 가능한 값만 쓴다)"];
}
