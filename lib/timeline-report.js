...
const REPORT_MODE_GUIDE = {
  direction: [
    ...
    "분석 단위는 '과거 상태 대 현재 상태'다. [기준 시점 대비표]와 정량 시계열에서 비교 가능한 가장 오래된 시점과 최근 시점을 고정하고, 사업 믹스·제품/화학계·고객·지역·투자/생산 중 실제로 무게가 옮겨 간 축만 고른다.",
    "쓰지 않는 것: 사건을 시간순으로 이어 붙인 서사(패턴 보고서의 일), 진행 중 사건의 갈림길(분기점 보고서의 일).",
    "출력 골격:",
    "# 전략 방향: (무게중심이 어디서 어디로 옮겨 갔는지를 담은 결론형 제목)",
    "## 핵심 요약",
    "- (3~4줄. 첫 줄은 '○○에서 ○○로' 무게중심이 옮겨 갔다는 판단, 이어서 그 이동을 보여 주는 대비·결정·한국 셀·소재사 함의를 한 줄씩.)",
    "## 과거 대 현재",
    "| 축 | 과거 (시점) | 현재 (시점) | 달라진 것 |  ← 무게가 옮겨 간 축만 2~4행. 칸마다 수치나 사건 하나.",
    "## 방향을 만든 결정",
    "최대 3개. '### 결론형 제목' 아래 두세 문장: 사실(날짜·수치) → 그것이 무게중심을 어떻게 옮겼는지.",
    "## Takeaway",
  ...
  pattern: [
    ...
    "패턴 유형: 수렴(여러 레이어가 같은 방향으로 모임) / 선행-후행(한 사건 뒤에 다른 레이어 사건이 이어짐, 시차를 개월로 적는다) / 괴리(이어질 흐름이 끊기거나 반대로 감) / 전환(반복되던 행동이 최근 달라짐). 실제로 성립하는 것만 최대 3개, 성립하는 것이 하나면 하나만.",
    "[레이어 간 연결 후보]는 기계적 후보다. 같은 제품·공장·고객·기술로 이어지는 것만 쓰고, 후보에 없는 연결도 사건 표에서 찾아도 된다.",
    "쓰지 않는 것: 과거 대 현재 표(전략 방향의 일), 경로 A/B(분기점의 일). 시간 순서는 '이어졌다'·'앞섰다'로 쓴다.",
    "출력 골격:",
    "# 패턴 인사이트: (패턴들을 관통하는 연결을 담은 결론형 제목)",
    "## 핵심 요약",
    "- (3~4줄. 첫 줄은 관통하는 연결, 이어서 핵심 패턴의 결론과 한국 셀·소재사 함의를 한 줄씩.)",
    "## 패턴 1: (결론형 제목)",
    "**유형:** 수렴 | 선행-후행 | 괴리 | 전환 중 하나",
    "**연결:** `YYYY-MM [레이어] 사건` → (N개월) → `YYYY-MM [레이어] 사건` 한 줄 사슬. 모든 사건은 입력에 있어야 한다.",
    "**이 연결이 말하는 것:** 두세 문장.",
    "(패턴 2, 3도 같은 형식.)",
    "## Takeaway",
  ...
  inflection_point: [
    ...
    "분석 단위는 '아직 끝나지 않은 사건'이다. [진행 단계 지도]에서 다음 단계가 남은 사업을 고르고, 끝난 실적은 현재 위치를 설명하는 데만 쓴다.",
    "경로는 예언이 아니다. 이미 관측된 움직임에서 바로 이어질 수 있는 두 갈래(예: 예정대로 양산 진입 대 일정 지연·축소)를 조건부로 쓴다. 성공 확률·미래 매출을 수치로 만들지 않는다.",
    "쓰지 않는 것: 과거 대 현재 표, 사건 사슬·패턴 유형, 할 일 목록 형태의 확인 과제.",
    "출력 골격:",
    "# 전략 분기점: (기업이 서 있는 갈림길을 담은 결론형 제목)",
    "## 핵심 요약",
    "- (3~4줄. 첫 줄은 현재 위치(무엇이 어디까지 진행됐고 무엇이 남았는지), 이어서 핵심 분기점과 갈림 신호, 한국 셀·소재사 함의를 한 줄씩.)",
    "## 분기점 1: (결론형 제목)",
    ...
    "## Takeaway",
    "- (한국 셀·양극재·음극재 사업에 주는 의미 2~3줄. 어느 갈림 신호를 보면 되는지 포함.)",
    "## 근거",
  ].join("\n"),
};
