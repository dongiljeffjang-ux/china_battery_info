...
// 화면에는 보조 데이터를 포함해 많은 사건을 그대로 보이되, 모델에 같은 시기·레이어의
// 반복 근거까지 모두 넘기면 지연과 시간 초과가 발생한다. 각 기간·레이어에서 양 끝의
// 사건을 우선 남기고, 그래도 많으면 시간축 전체에서 고르게 뽑는다. 이 선택은 분석 근거를
// 새로 만들거나 순위를 매기지 않으며, 화면의 원시 이벤트를 바꾸지도 않는다.
export function selectTimelineEvidence(events, { limit = 48, perBucket = 2 } = {}) {
  const ordered = [...(events || [])].sort((a, b) => String(a.date).localeCompare(String(b.date)) || String(a.id).localeCompare(String(b.id)));
  ...
  for (const bucket of buckets.values()) {
    representative.push(...bucket.slice(0, perBucket));
    if (bucket.length > perBucket) representative.push(...bucket.slice(-perBucket));
  }
  const unique = [...new Map(representative.map(event => [event.id, event])).values()]
    .sort((a, b) => String(a.date).localeCompare(String(b.date)) || String(a.id).localeCompare(String(b.id)));
  if (unique.length <= limit) return unique;
  // 첫·마지막 시점은 반드시 남기고, 중간은 시간축 전체에서 균등하게 고른다.
...
export function buildTimelineInput({ companyName, events, metrics = [], policies = [] }) {
  ...
  const selectedEvents = selectTimelineEvidence(events, { limit: 24, perBucket: 1 }).map(event => ({
    ...event,
  ...
  const selectedPolicies = selectReportPolicies(policies, selectedEvents);
  ...
  return [
    ...
    quant || "(없음)",
    "",
    `화면 이벤트 ${events.length}건 중 기간·레이어별 대표 근거 ${selectedEvents.length}건(입력 예산 내 압축, 과거 → 최근):`,
    "",
...
export async function buildTimelineReport({ companyName, events, metrics = [], policies = [] }) {
  const { data, model } = await createJsonResponse({
    name: "company_timeline_report",
    provider: "openai_report",
    timeoutMs: 110000,
    schema: REPORT_SHAPE,
    instructions: `${TIMELINE_REPORT_PROMPT}${policies.length ? `\n${POLICY_ANALYSIS_RULE}` : ""}`,
    input: buildTimelineInput({ companyName, events, metrics, policies }),
  });
