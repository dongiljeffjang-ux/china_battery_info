...
const UNIFIED_REPORT_GUIDE = [
  ...
  "쓰기에 앞서 보고서 전체를 관통하는 중심 논지 한 문장을 내부적으로 정하라: '과거의 중심축 → 누적된 변화 → 현재 위치 → 그래서 한국 셀·소재사에 생기는 사업적 의미'. 이 흐름에 기여하지 않는 사건은 본문에 넣지 마라.",
  "세 관점을 아래 순서로 연결하되 같은 사실과 결론을 절마다 반복하지 마라. 앞 절의 마지막 판단이 다음 절의 출발점이 되어야 한다. 사건을 연대순으로 나열한 뒤 마지막에 함의를 덧붙이는 방식으로 쓰지 마라.",
  ...
  "2. 반복 실행 방식은 앞에서 확인한 변화가 어떤 방식으로 축적됐는지를 설명해야 한다. 서로 독립된 제품·공장·지역·프로젝트 두 사례 이상에서 같은 구체적 선택이 확인될 때만 쓴다. 같은 프로젝트의 개발·착공·인증·출하는 하나의 사례이며, '개발 → 생산 → 출하' 같은 제조업의 일반 순서는 패턴이 아니다. 성립하는 패턴이 없으면 이 절을 뺀다.",
  "3. 핵심 변수는 이미 확인된 진행 상태에서 기업의 역할·자원 배분·고객 기반과 마지막 So What 판단에 큰 영향을 줄 아직 미결인 항목이다. 미래를 두 개 시나리오로 나누지 마라. 각 변수마다 현재 확인된 사실, 왜 중요한지, 향후 시계열에서 확인할 수 있는 구체적 신호만 적는다. 신호가 없다는 사실을 실패나 반대 경로의 확정으로 쓰지 않는다.",
  "4. 모든 주요 해석은 '사실이 무엇인가 → 그 사실들이 함께 보여 주는 변화는 무엇인가 → 그래서 독자의 사업 판단에 무엇이 달라지는가'까지 이어져야 한다. 단, 입력에 없는 대응책이나 투자 행동을 권고하지 마라.",
  ...
  "> **한 문장 결론:** (과거 중심축 → 누적된 변화 → 현재 위치 → 그래서 한국 셀·소재사에 중요한 이유를 하나의 문장으로 쓴다.)",
  "## 핵심 요약",
  "- (3~4줄. 무엇이 달라졌나 → 변화가 어떻게 축적됐나 → 현재 무엇으로 판정하나 → So What 순서로 쓴다. 근거 없는 항목으로 줄 수를 채우지 않는다.)",
  "## 1. 출발점 — 무엇이 전략의 중심이었나",
  "비교가 시작되는 가장 이른 상태를 한두 문장으로 확정한다. 오래된 사건을 모두 나열하지 말고 이후 변화와 대비되는 사업·제품·지역·생산기반만 고른다.",
  "## 2. 변화의 축적 — 무엇이 더해지고 달라졌나",
  "결론에 영향을 준 움직임만 과거 → 최근 순으로 최대 3개 연결한다. 각 항목은 '### 결론형 제목' 아래 시점·사실, 앞선 상태에 더해진 변화, 전략적 의미를 두세 문장으로 쓴다. 시간적 선후를 인과로 바꾸지 않는다.",
  "## 3. 현재 위치 — 유지·확장·중심 이동 중 어디인가",
  "| 변화 축 | 출발점 (시점) | 현재 (시점) | 판정 |  ← 비교 가능한 축만 최대 3행.",
  "표 뒤에 앞 절의 누적 변화가 왜 이 판정으로 이어지는지 두세 문장으로 종합한다.",
  "### 이 흐름에서 반복된 실행 방식",
  "앞 절의 변화를 관통하며 성립하는 패턴만 최대 2개 쓴다. 각 패턴은 서로 다른 독립 사례 최소 2개의 시점·사실, 공통 선택, 적용 범위를 두세 문장으로 쓴다. 성립하는 패턴이 없으면 이 소제목 전체를 뺀다.",
  "## 4. 현재 판단을 바꿀 핵심 변수",
  ...
  "## So What — 한국 배터리·소재사에 무엇이 달라지는가",
  "- (관련 있는 셀·양극재·음극재 부문에 한정해 2~3줄. 경쟁 조건·고객 기회·공급망 선택 가운데 실제로 달라지는 것을 먼저 쓰고, 유리·압박·중립 방향과 시계열 근거를 붙인다.)",
  "- 앞 절을 요약해서 반복하지 마라. 이 기업의 변화 때문에 한국 기업이 보던 시장·고객·기술·공급망의 전제가 무엇에서 무엇으로 바뀌는지를 답하라.",
  "## 근거",
].join("\n");
