...
const REPORT_MODE_GUIDE = {
  direction: "[선택한 보고서 용도: 전략 방향]\n이 절은 앞선 일반 목차보다 우선한다. 질문은 '이 기업은 지금 어디에 무게를 두고 있으며 과거와 무엇이 달라졌는가'다. 정량 흐름과 제품·고객·지역·투자의 변화를 연결해 현재 전략 판단을 2문장으로 먼저 제시하라. 그 뒤 방향을 만든 변화는 최대 3개만, '과거 상태 → 현재 변화 → 사업적 의미'로 쓴다. 출력은 '# 전략 방향', '## 현재 판단', '## 방향을 만든 변화', '## 한국 산업에 주는 의미', '## 근거' 순서의 Markdown이다. '확인된 것 / 아직 모르는 것 / 다음 확인 지표'를 의무 섹션으로 만들지 마라.",
  pattern: "[선택한 보고서 용도: 패턴 인사이트]\n이 절은 앞선 일반 목차보다 우선한다. 질문은 '개별 사건만 읽을 때는 보이지 않던 연결은 무엇인가'다. Convergence, Lead-Lag, Gap / Divergence, Inflection 중 실제로 성립하는 패턴만 최대 3개 고른다. 단일 사건은 패턴이 아니다. 같은 제품·사업의 앞뒤 사건 또는 서로 다른 두 레이어·시점을 연결하라. 각 패턴에서 무엇이 연결되는지와 기업·한국 산업에 주는 의미를 결론형 제목으로 쓴다. 반복되는 유보 문구는 쓰지 않는다. 출력은 '# 패턴 인사이트', '## Strategic Thesis', '## Top Patterns', '## 한국 산업에 주는 의미', '## 근거' 순서의 Markdown이다. '확인된 것 / 아직 모르는 것 / 다음 확인 지표'를 의무 섹션으로 만들지 마라.",
  inflection_point: "[선택한 보고서 용도: 전략 분기점]\n이 절은 앞선 일반 목차보다 우선한다. 질문은 '진행 중인 변화가 어떤 경로로 갈릴 수 있으며 각 경로가 무엇을 의미하는가'다. 계획·인증·고객 지정·건설·시험생산·해외 거점처럼 아직 진행 중인 중요한 사건을 중심으로 최대 3개 분기점을 고른다. 각 분기점은 현재 위치, 경로 A의 전략적 의미, 경로 B의 전략적 의미, 두 경로를 가르는 구체적 신호를 쓴다. 분기 신호를 확인 과제나 할 일 목록처럼 쓰지 말고, 신호가 나타나면 판단이 어떻게 바뀌는지를 함께 쓴다. 출력은 '# 전략 분기점', '## 현재 위치', '## 핵심 분기점', '## 한국 산업에 주는 의미', '## 근거' 순서의 Markdown이다. '확인된 것 / 아직 모르는 것 / 다음 확인 지표'를 의무 섹션으로 만들지 마라."
};
...
export function buildTimelineInput({ companyName, companyTags = [], reportMode = "direction", events, metrics = [], alternatives = [], policies = [] }) {
  ...
  const selectedPolicies = selectReportPolicies(policies, selectedEvents, { companyTags });
  const policySection = selectedPolicies.length ? [
    "",
    `같은 기간의 중국 배터리·NEV·ESS 대표 정책 ${selectedPolicies.length}건(전역 정책축, 과거 → 최근):`,
    "",
    policyEvidenceText(selectedPolicies, { compact: true }),
    POLICY_ANALYSIS_RULE,
    "",
    "정책은 회사 사건과 구분하되, 시행 시점과 적용 대상이 회사의 제품·수요·원가·생산·수출 조건에 닿는 경우에만 사업 관련성을 분석하라. 단순한 시간적 선후를 인과로 단정하지 마라.",
  ] : [];
  return [
    "[입력 자료]",
    "",
...
export async function buildTimelineReport({ companyName, companyTags = [], reportMode = "direction", events, metrics = [], alternatives = [], policies = [] }) {
  ...
  const { data, model } = await createJsonResponse({
    ...
    timeoutMs: 110000,
    schema: REPORT_SHAPE,
    instructions: `${TIMELINE_REPORT_PROMPT}\n\n${REPORT_MODE_GUIDE[mode]}${policies.length ? `\n${POLICY_ANALYSIS_RULE}` : ""}`,
    input: buildTimelineInput({ companyName, companyTags, reportMode: mode, events, metrics, alternatives, policies }),
  });
