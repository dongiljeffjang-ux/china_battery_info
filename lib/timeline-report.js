...
export function selectTimelineEvidence(events, { limit = 48, perBucket = 2 } = {}) {
  ...
  const fixedIds = new Set(fixed.map(event => event.id));
  const rest = unique.filter(event => !fixedIds.has(event.id)).sort(byPriorityThenDate);
  return [...fixed, ...rest.slice(0, Math.max(0, limit - fixed.length))]
    .sort((a, b) => String(a.date).localeCompare(String(b.date)) || String(a.id).localeCompare(String(b.id)));
}
...
export function buildTimelineInput({ companyName, companyTags = [], events, metrics = [], policies = [] }) {
...
export async function buildTimelineReport({ companyName, companyTags = [], events, metrics = [], policies = [] }) {
  const { data, model } = await createJsonResponse({
    ...
    input: buildTimelineInput({ companyName, companyTags, events, metrics, policies }),
  });
