...
export const INSTRUCTIONS = `당신은 중국 배터리 산업 이벤트 문장에서 구조화 사실을 뽑는 분석가다. 문장에 적힌 것만 옮기고, 계산·환산·추정을 하지 않는다.
...
export async function extractFactsForEvents(events, { provider = "auto" } = {}) {
  ...
  const result = await createJsonResponse({ name: "event_fact_extraction", schema: factSchema(), instructions: INSTRUCTIONS, input, provider, timeoutMs: CALL_BUDGET_MS });
  const rows = [];
  const dropped = {};
...
export async function extractMissingFacts({ deadline = Infinity, provider = "auto" } = {}) {
  ...
  for (let i = 0; i < Math.min(pending.length, EVENTS_PER_CALL * MAX_CALLS_PER_HOP); i += EVENTS_PER_CALL) {
    // 첫 배치는 무조건 돈다. 남은 예산으로만 판단하면 앞 단계가 몇 초만 써도 한 건도 못 한다.
    if (i > 0 && Date.now() > deadline - CALL_BUDGET_MS) break;
...
export const FACT_EXTRACTION_PROMPT = INSTRUCTIONS;
