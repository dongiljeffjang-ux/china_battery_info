...
export const EVENT_DISPLAY_SUMMARY_VERSION = "v2";
...
- 개조식 한 줄(명사형·간결한 서술형 허용), 60자 이하로 쓴다. 불릿 기호·제목·출처·'원문' 표기는 쓰지 않는다.
...
export async function summarizeEventDisplayBatch({ limit = EVENT_DISPLAY_SUMMARY_BATCH_SIZE } = {}) {
  ...
  const events = await supabaseRest(`event?select=id,title_ko,fact_ko,original_excerpt_ko&or=(display_summary_ko.is.null,display_summary_version.is.null,display_summary_version.neq.${EVENT_DISPLAY_SUMMARY_VERSION})&evidence_kind=in.(${REPORT_EVENT_KINDS})&order=occurred_at.desc,id.asc&limit=${batchSize}`);
  ...
  await Promise.all([...updates].map(([id, summary]) => supabaseRest(`event?id=eq.${encodeURIComponent(id)}`, {
    ...
    body: {
      ...
      display_summary_version: EVENT_DISPLAY_SUMMARY_VERSION,
