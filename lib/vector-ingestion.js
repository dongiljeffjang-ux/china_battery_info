...
// 기사 청크의 본문 구성. 요약은 첫 조각에만 넣고, 본문 조각에는 제목 한 줄만 붙인다.
//
// 왜 바꿨나(2026-09-09 실측). 예전에는 청크마다 `[한국어 팩트 요약] 제목+요약`을 통째로 복사해
// 붙였다. 그래서 긴 기사 하나가 21개 청크가 되면 21개 전부가 같은 수백 자로 시작했고, 의미 검색과
// 단어 검색 양쪽에 그 기사만 무더기로 걸렸다. "전고체 배터리 준비중인 회사들"에 근거 10건 중 6건이
// 같은 SMM 기사였고, 답변 모델은 회사 두 곳 이야기만 받고 "근거 부족"이라 답했다. 전체 기사 청크의
// 61.2%가 같은 기사의 다른 청크와 앞 160자가 같았다.
//
// 제목 한 줄은 남긴다. 본문 조각만 남기면 그 조각이 어느 회사·무슨 사건 이야기인지 알 수 없어
// 회사 스코핑(knowledge-search의 본문 표기 검사)과 사람이 읽는 근거 목록이 둘 다 망가진다.
export const ARTICLE_SUMMARY_LABEL = "[한국어 팩트 요약]";
export const ARTICLE_TITLE_LABEL = "[기사]";
export const ARTICLE_BODY_LABELS = { translated: "[한국어 본문]", original: "[원문 근거 청크]" };

export function buildArticleChunkTexts({ titleKo, summaryKo, bodyChunks = [], bodyLabel = ARTICLE_BODY_LABELS.original }) {
  const title = String(titleKo || "").trim();
  const summary = [ARTICLE_SUMMARY_LABEL, title, String(summaryKo || "").trim()].filter(Boolean).join("\n").trim();
  if (!bodyChunks.length) return summary.length >= 20 ? [summary] : [];
  const body = bodyChunks.map((chunk) => `${ARTICLE_TITLE_LABEL} ${title}\n\n${bodyLabel}\n${chunk}`);
  return summary.length >= 20 ? [summary, ...body] : body;
}

// 저장된 청크 텍스트를 제목·요약·본문으로 되돌린다. 옛 형식(요약이 모든 조각에 붙은 것)과
// 새 형식을 모두 읽는다 — 마이그레이션이 두 번 돌아도 결과가 같아야 한다.
export function splitArticleChunkText(text) {
  const source = String(text || "");
  for (const [kind, label] of Object.entries(ARTICLE_BODY_LABELS)) {
    const at = source.indexOf(`\n\n${label}\n`);
    if (at < 0) continue;
    const head = source.slice(0, at);
    const body = source.slice(at + label.length + 3);
    if (head.startsWith(`${ARTICLE_TITLE_LABEL} `)) {
      return { title: head.slice(ARTICLE_TITLE_LABEL.length + 1).trim(), summary: "", body, bodyLabel: label, kind };
    }
    const [, title = "", ...rest] = head.split("\n");
    return { title: title.trim(), summary: rest.join("\n").trim(), body, bodyLabel: label, kind };
  }
  // 본문 라벨이 없으면 요약 한 조각이다.
  const [, title = "", ...rest] = source.split("\n");
  return { title: title.trim(), summary: rest.join("\n").trim(), body: "", bodyLabel: null, kind: null };
}
...
${summaryKo || ""}`.trim();
    ...
    const bodyChunks = translation.paragraphs.length
      ...
      : [];
    const inputs = buildArticleChunkTexts({
      titleKo: titleKo || article.title_original, summaryKo, bodyChunks, bodyLabel: ARTICLE_BODY_LABELS.translated,
    });
    if (!inputs.length) return { chunks: 0, status: "skipped" };
    const { model, embeddings } = await createEmbeddings(inputs);
    const rows = inputs.map((content, index) => ({
      ...
      content_hash: crypto.createHash("sha256").update(`${article.id}:ko:${index}:${content}`).digest("hex"),
  ...
  // 요약은 첫 조각에만 넣는다. 예전에는 조각마다 복사해 붙여 한 기사가 검색 결과를 독점했다.
  const inputs = buildArticleChunkTexts({
    titleKo: titleKo || article.title_original, summaryKo, bodyChunks: originalChunks, bodyLabel: ARTICLE_BODY_LABELS.original,
  });
  if (!inputs.length) return { chunks: 0, status: "skipped" };
  const { model, embeddings } = await createEmbeddings(inputs);
  const total = inputs.length;
  // 첫 조각이 요약이면 본문 조각은 한 칸씩 밀린다. content_original은 본문 조각에만 붙는다.
  const summaryFirst = total === originalChunks.length + 1;
  const rows = inputs.map((content, index) => ({
    ...
    published_at: publishedAt ? String(publishedAt).slice(0, 10) : null,
    content_ko: content,
    content_original: summaryFirst && index === 0 ? null : originalChunks[summaryFirst ? index - 1 : index],
    chunk_index: index,
    chunk_total: total,
    content_hash: crypto.createHash("sha256").update(`${article.id}:${index}:${content}`).digest("hex"),
    embedding: embeddings[index],
    embedding_model: model
  }));
  // 다시 임베딩하면 옛 조각이 남아 겹친다. content_hash가 텍스트를 담고 있어 형식이 바뀌면 새 행이 되므로,
  // 이 기사의 조각을 비우고 새로 넣는다. 다른 기사·다른 source_type은 건드리지 않는다.
  await supabaseRest(`knowledge_chunk?article_id=eq.${encodeURIComponent(article.id)}&source_type=eq.article_chunk`, { method: "DELETE", prefer: "return=minimal" }).catch(() => {});
  await supabaseRest("knowledge_chunk?on_conflict=content_hash", {
    method: "POST", prefer: "resolution=merge-duplicates,return=minimal", body: rows
  });
