    ...
    verification: {
      ...
      properties: {
        ...
        added_evidence: {
          ...
          items: {
            type: "object", additionalProperties: false,
            required: ["company", "occurred_at", "occurred_precision", "title_ko", "fact_ko", "trajectory_track", "source_name", "source_url"],
            properties: {
...
const FACT_RULE =
  ...
  "strategy와 timeline은 사실 정리이므로 해석을 섞지 않는다. korea_insight만 해석이다. " +
...
const VERIFY_RULE =
  ...
  "added_evidence에는 검색으로 새로 확인한 사실만 넣는다. 입력 이벤트에 이미 있는 사실은 넣지 않는다. company는 그 사실의 주체(A 또는 B), occurred_at은 실제 발생일(YYYY-MM-DD), occurred_precision은 그 날짜를 어디까지 믿는지, trajectory_track은 시장(market)·기술(technology)·둘 다(both) 가운데 하나다. " +
  ...
  if (report.verification) {
    out.verification = {
      ...
      added_evidence: (report.verification.added_evidence || []).map((item) => ({
        ...
        trajectory_track: ["market", "technology", "both"].includes(item.trajectory_track) ? item.trajectory_track : "market",
...
function serializeEvents(label, name, events) {
  ...
  const lines = events.map((event) =>
    `- id=${event.id || "-"} | ${event.date || "시점미상"} | ${String(event.title || "").slice(0, 120)} | ${String(event.fact || "").replace(/\s+/g, " ").slice(0, 240)} | 출처: ${event.sourceName || "미상"}`
  );
...
export async function buildCompareReport({ nameA, nameB, eventsA, eventsB }) {
  ...
  const draft = await createJsonResponse({
    ...
    provider: "openai",
    instructions:
      `당신은 중국 이차전지 산업 분석가다. 두 회사 ${nameA}(A)와 ${nameB}(B)를 비교하는 리포트를 한국어로 쓴다. ` +
      `${FACT_RULE} ${INSIGHT_RULE} ${LENGTH_GUIDE}`,
    input: `비교 대상 A: ${nameA}\n비교 대상 B: ${nameB}\n\n화면에 표시된 근거 이벤트:\n${evidence}`
  });
  ...
  try {
    const verified = await createJsonResponse({
      ...
      provider: "openai",
      webSearch: true,
      instructions: `${VERIFY_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${LENGTH_GUIDE}`,
...
export async function applyVerifiedFacts({ companyA, companyB, report, knownEventIds }) {
  ...
  for (const item of verification.added_evidence || []) {
    ...
    rows.push({
      ...
      trajectory_track: item.trajectory_track,
      layer_key: null,
      region_scope: null,
