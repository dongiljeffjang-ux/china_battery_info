...
const REPORT_SHAPE = {
  ...
  required: ["headline_ko", "trajectory", "comparison", "korea_insight"],
  properties: {
...
const TRAJECTORY_RULE =
  ...
  "comparison은 1단계에서 파악한 두 궤적을 축별로 견준다. market_ko는 두 회사의 시장 궤적이 어떻게 다른지, technology_ko는 기술 궤적이 어떻게 다른지, divergence_ko는 두 회사가 결정적으로 갈라지는 지점을 짚는다.";
...
const FACT_RULE =
  ...
  "trajectory와 comparison은 사실 정리이므로 해석을 섞지 않는다. korea_insight만 해석이다. " +
...
export async function buildCompareReport({ companyIdA, companyIdB, nameA, nameB, eventsA, eventsB }) {
  ...
  const evidence = [serializeEvents("A", nameA, eventsA), serializeEvents("B", nameB, eventsB), conceptGraphText].filter(Boolean).join("\n\n");
  ...
  const draft = await createJsonResponse({
    ...
    instructions:
      ...
      `${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE}`,
    input: `비교 대상 A: ${nameA}\n비교 대상 B: ${nameB}\n\n화면에 표시된 근거 이벤트:\n${evidence}`
  });
  ...
  try {
    const verified = await createJsonResponse({
      ...
      provider: "openai",
      webSearch: true,
      instructions: `${VERIFY_RULE} ${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE}`,
