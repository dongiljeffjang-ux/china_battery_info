...
import { metricTable } from "./timeline-report.js";
import { POLICY_ANALYSIS_RULE, policyEvidenceText } from './policy-context.js';
...
export async function buildCompareReport({ companyIdA, companyIdB, nameA, nameB, eventsA, eventsB, metricsA = [], metricsB = [], policies = [], pairContext }) {
  ...
  const evidence = [
    ...
    policies.length ? '[중국 정책축]\n' + policyEvidenceText(policies) : "",
    policies.length ? POLICY_ANALYSIS_RULE : "",
    conceptGraphText
  ].filter(Boolean).join("\n\n");
  ...
  try {
    const verified = await createJsonResponse({
      ...
      instructions: `${VERIFY_RULE} ${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${PAIR_LITE_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE}${policies.length ? ` ${POLICY_ANALYSIS_RULE}` : ""}`,
