...
import { metricTable, selectTimelineEvidence } from "./timeline-report.js";
...
function serializeMetrics(label, name, metrics) {
  const table = metricTable(metrics);
...
export async function buildCompareReport({ companyIdA, companyIdB, nameA, nameB, companyTags = [], eventsA, eventsB, metricsA = [], metricsB = [], policies = [], pairContext }) {
  ...
  const evidence = [
    ...
    serializeMetrics("A", nameA, metricsA), serializeEvents("A", nameA, eventsA),
    serializeMetrics("B", nameB, metricsB), serializeEvents("B", nameB, eventsB),
