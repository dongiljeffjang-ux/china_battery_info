...
export async function buildCompareReport({ companyIdA, companyIdB, nameA, nameB, companyTags = [], eventsA, eventsB, metricsA = [], metricsB = [], alternativesA = [], alternativesB = [], policies = [], pairContext }) {
  ...
  const draft = await createJsonResponse({
    ...
    instructions:
      ...
      `${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${PAIR_LITE_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE}${policyInstruction}`,
    input: `비교 대상 A: ${nameA}\n비교 대상 B: ${nameB}\n\n화면에 표시된 근거 이벤트:\n${evidence}`
  });
  ...
  try {
    const verified = await createJsonResponse({
      ...
      instructions: `${VERIFY_RULE} ${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${PAIR_LITE_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE}${selectedPolicies.length ? ` ${POLICY_ANALYSIS_RULE}` : ""}`,
