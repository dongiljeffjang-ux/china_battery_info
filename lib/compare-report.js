...
export async function buildCompareReport({ companyIdA, companyIdB, nameA, nameB, eventsA, eventsB, metricsA = [], metricsB = [], policies = [], pairContext }) {
  ...
  const evidence = [
    ...
    serializeMetrics("B", nameB, metricsB), serializeEvents("B", nameB, eventsB), '[중국 정책축]\n' + policyEvidenceText(policies), POLICY_ANALYSIS_RULE, conceptGraphText
  ].filter(Boolean).join("\n\n");
  ...
  const draft = await createJsonResponse({
    ...
    instructions:
      ...
      `${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${PAIR_LITE_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE} 중국 정책축은 두 회사에 공통으로 작용할 수 있는 외부 변수다. 적용 대상·시행 시점이 각 회사의 제품·수요·원가·생산·수출 조건에 실제로 닿는 경우에만 trajectory와 comparison에 포함하고, 시간적 선후를 인과로 단정하지 마라.`,
    input: `비교 대상 A: ${nameA}\n비교 대상 B: ${nameB}\n\n화면에 표시된 근거 이벤트:\n${evidence}`
  });
  ...
  try {
    const verified = await createJsonResponse({
      ...
      instructions: `${VERIFY_RULE} ${TRAJECTORY_RULE} ${AXIS_RULE} ${FACT_RULE} ${INSIGHT_RULE} ${PAIR_LITE_RULE} ${LENGTH_GUIDE} ${CONCEPT_GRAPH_RULE} ${POLICY_ANALYSIS_RULE}`,
