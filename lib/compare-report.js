...
function serializeEvents(label, name, events, alternatives = []) {
  ...
  const lines = selected.map((event) =>
    ...
    `- id=${event.id || "-"} | [track=${trackLabel(event)}] | ${event.date || "시점미상"} | ${String(event.title || "").slice(0, 100)} | ${String(event.fact || "").replace(/\s+/g, " ").slice(0, 300)}${event.entity ? ` | 발생 법인: ${String(event.entity).slice(0, 80)}` : ""} | 출처: ${String(event.sourceName || "미상").slice(0, 70)}` +
