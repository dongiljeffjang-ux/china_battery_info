...
- 기업 페이지의 분석 생성 기능은 세부 용도별 보고서로 나누지 않고 하나의 **시계열 분석 보고서**로 제공한다. 화면의 약 40~50개 팩트와 정량 시계열을 함께 읽어 전략 변화, 독립 사례에서 반복된 실행 방식, 향후 판단을 바꿀 관측 가능한 핵심 변수, 한국 배터리·소재사에 주는 의미를 한 흐름으로 정리한다. 핵심 변수는 근거가 약한 양자택일 시나리오로 제시하지 않는다.
