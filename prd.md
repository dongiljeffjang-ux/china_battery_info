...
배터리·NEV·ESS 정책을 매일 추가 수집한다. 과거 정책은 사용자 첨부 연혁을 사용하며 별도 과거 수집은 하지 않는다. 기업분석·비교 시간축에 공통 정책을 표시하고, 두 페이지의 보고서 생성 시 정책 시점·적용 대상을 분석 변수로 포함한다. 사용자 제공 연혁과 독립 원문 검증은 구분한다. 구현·출처 범위는 `docs/POLICY-AXIS.md` 참고.

> 구현 현황과 운영상의 최신 결정은 `docs/HANDOFF.md`가 우선한다. 본 문서는 제품 요구의 기준이다.
...
- 중국어·영문 원문은 접근통제된 내부 DB에 보관해 청킹·임베딩·팩트 대조에 사용한다. 외부 재배포는 하지 않으며 보존기간·저작권 정책은 별도로 확정한다.
...
- 기업 페이지의 분석 생성 기능은 세부 용도별 보고서로 나누지 않고 하나의 **시계열 분석 보고서**로 제공한다. 화면의 약 40~50개 팩트와 정량 시계열을 함께 읽어 과거 중심축 → 누적된 변화 → 현재 위치와 반복 실행 방식 → 현재 판단을 바꿀 핵심 변수 → 한국 배터리·소재사에 무엇이 달라지는가라는 흐름으로 정리한다. 핵심 변수는 근거가 약한 양자택일 시나리오로 제시하지 않으며, 마지막 `So What`은 경쟁 조건·고객 기회·공급망 선택 중 실제로 달라지는 전제를 답해야 한다.
...
- 관리자는 읽기 전용 데이터 감사 화면에서 전체 기사·이벤트·지식 청크의 회사 귀속과 참조 관계를
  자동 점검하고, 높은 우선도와 확인 필요 후보를 필터링하거나 CSV로 내려받아 원문 검토할 수 있다.
  자동 감사는 후보를 제시할 뿐 데이터를 자동 수정하지 않는다.
