...
const { buildSearchGroups, plannedSearchRequests, COMPANIES } = await import('../lib/china-sources.js');
...
for (const provider of ['openai', 'deepseek']) {
  ...
  for (const company of COMPANIES) if (['cell','cathode','anode'].some((t) => company.type_tags.includes(t))) assert.ok(covered.has(company.id), `${company.id} covered by ${provider}`);
}
