...
assert.match(html, /전략의 중심은\?/, "전략 방향은 사용자가 바로 이해할 질문형 표현을 써야 한다");
assert.match(html, /반복 신호는\?/, "패턴 인사이트는 사용자가 바로 이해할 질문형 표현을 써야 한다");
assert.match(html, /판을 가를 변수는\?/, "전략 분기점은 사용자가 바로 이해할 질문형 표현을 써야 한다");
